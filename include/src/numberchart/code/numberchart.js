...
(function (upred) {
    ...
    (function (math) {
        ...
        (function (NumberDirection) {
            NumberDirection[NumberDirection["FROM_TOPLEFT_TO_RIGHT"] = 0] = "FROM_TOPLEFT_TO_RIGHT";
            NumberDirection[NumberDirection["FROM_TOPLEFT_TO_BOTTOM"] = 1] = "FROM_TOPLEFT_TO_BOTTOM";
            NumberDirection[NumberDirection["FROM_BOTTOMLEFT_TO_RIGHT"] = 2] = "FROM_BOTTOMLEFT_TO_RIGHT";
            NumberDirection[NumberDirection["FROM_BOTTOMLEFT_TO_TOP"] = 3] = "FROM_BOTTOMLEFT_TO_TOP";
            NumberDirection[NumberDirection["FROM_TOPRIGHT_TO_LEFT"] = 4] = "FROM_TOPRIGHT_TO_LEFT";
            NumberDirection[NumberDirection["FROM_TOPRIGHT_TO_BOTTOM"] = 5] = "FROM_TOPRIGHT_TO_BOTTOM";
            NumberDirection[NumberDirection["FROM_BOTTOMRIGHT_TO_LEFT"] = 6] = "FROM_BOTTOMRIGHT_TO_LEFT";
            NumberDirection[NumberDirection["FROM_BOTTOMRIGHT_TO_TOP"] = 7] = "FROM_BOTTOMRIGHT_TO_TOP";
        })(NumberDirection || (NumberDirection = {}));
        var ChartGroup = (function () {
            function ChartGroup(canvas) {
                this.cellNumbers = [];
                this.cellNumberTags = [];
                this.cellColors = [];
                this.cellMarks = [];
                this.numStart = 1;
                this.direction = NumberDirection.FROM_TOPLEFT_TO_RIGHT;
                this.numAdd = 1;
                this.numMultiple = 1;
                this.cellSize = 50;
            ...
            ChartGroup.prototype.MakeTable = function (colN, rowN) {
                this.tableRootTag = document.createElementNS('http://www.w3.org/2000/svg', 'g');
                this.groupTag.appendChild(this.tableRootTag);
                this.tableRootTag.style.pointerEvents = 'none';
                this.colorsBackTag = document.createElementNS('http://www.w3.org/2000/svg', 'g');
                this.groupTag.appendChild(this.colorsBackTag);
                this.colorsBackTag.style.pointerEvents = 'none';
                this.marksBackTag = document.createElementNS('http://www.w3.org/2000/svg', 'g');
                this.groupTag.appendChild(this.marksBackTag);
                this.marksBackTag.style.pointerEvents = 'none';
                upred.ui.mathModule.AdjustCellSize(this, colN, rowN);
                var rectTag = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
                rectTag.setAttribute('x', '0');
                rectTag.setAttribute('y', '0');
                rectTag.setAttribute('width', (this.cellSize * colN).toString());
                rectTag.setAttribute('height', (this.cellSize * rowN).toString());
                rectTag.setAttribute('style', 'fill:white; touch-action: none;');
                rectTag.style.pointerEvents = 'all';
                this.tableRootTag.appendChild(rectTag);
                for (var i = 0; i <= colN; i++) {
                    var x = i * this.cellSize;
                    var lineTag = document.createElementNS('http://www.w3.org/2000/svg', 'line');
                    lineTag.setAttribute('x1', x.toString());
                    lineTag.setAttribute('y1', '0');
                    lineTag.setAttribute('x2', x.toString());
                    lineTag.setAttribute('y2', (this.cellSize * rowN).toString());
                    lineTag.setAttribute('style', 'touch-action: none;');
                    lineTag.setAttribute('stroke', 'black');
                    this.tableRootTag.appendChild(lineTag);
                }
                for (var i = 0; i <= rowN; i++) {
                    ...
                    var lineTag = document.createElementNS('http://www.w3.org/2000/svg', 'line');
                    lineTag.setAttribute('x1', '0');
                    lineTag.setAttribute('y1', y.toString());
                    lineTag.setAttribute('x2', (this.cellSize * colN).toString());
                    lineTag.setAttribute('y2', y.toString());
                    lineTag.setAttribute('style', 'touch-action: none;');
                    lineTag.setAttribute('stroke', 'black');
                    this.tableRootTag.appendChild(lineTag);
                }
                ...
                switch (this.direction) {
                    case NumberDirection.FROM_TOPLEFT_TO_RIGHT:
                        {
                            for (var row = 0; row < rowN; row++) {
                                for (var col = 0; col < colN; col++) {
                                    this.cellNumbers[row][col] = N;
                                    N = N * this.numMultiple + this.numAdd;
                                }
                            }
                        }
                        break;
                    case NumberDirection.FROM_TOPLEFT_TO_BOTTOM:
                        {
                            for (var col = 0; col < colN; col++) {
                                for (var row = 0; row < rowN; row++) {
                                    this.cellNumbers[row][col] = N;
                        ...
                        break;
                    case NumberDirection.FROM_BOTTOMLEFT_TO_RIGHT:
                        {
                        ...
                        break;
                    case NumberDirection.FROM_BOTTOMLEFT_TO_TOP:
                        {
                            for (var col = 0; col < colN; col++) {
                                for (var row = rowN - 1; row >= 0; row--) {
                                    this.cellNumbers[row][col] = N;
                        ...
                        break;
                    case NumberDirection.FROM_TOPRIGHT_TO_LEFT:
                        {
                            for (var row = 0; row < rowN; row++) {
                                for (var col = colN - 1; col >= 0; col--) {
                                    this.cellNumbers[row][col] = N;
                                    N = N * this.numMultiple + this.numAdd;
                                }
                            }
                        ...
                        break;
                    case NumberDirection.FROM_TOPRIGHT_TO_BOTTOM:
                        {
                            for (var col = colN - 1; col >= 0; col--) {
                                for (var row = 0; row < rowN; row++) {
                                    this.cellNumbers[row][col] = N;
                        ...
                        break;
                    case NumberDirection.FROM_BOTTOMRIGHT_TO_LEFT:
                        {
                            for (var row = rowN - 1; row >= 0; row--) {
                                for (var col = colN - 1; col >= 0; col--) {
                                    this.cellNumbers[row][col] = N;
                        ...
                        break;
                    case NumberDirection.FROM_BOTTOMRIGHT_TO_TOP:
                        {
                            for (var col = colN - 1; col >= 0; col--) {
                                for (var row = rowN - 1; row >= 0; row--) {
                                    this.cellNumbers[row][col] = N;
                ...
                this.textRootTag = document.createElementNS('http://www.w3.org/2000/svg', 'g');
                this.groupTag.appendChild(this.textRootTag);
                this.textRootTag.style.pointerEvents = 'none';
                for (var row = 0; row < rowN; row++) {
                    this.cellNumberTags[row] = [];
                    this.cellColors[row] = [];
                    this.cellMarks[row] = [];
                    for (var col = 0; col < colN; col++) {
                        var ttag = upred.ui.SVGSymbol.MakeTextTag(this.cellSize * col + this.cellSize / 2, this.cellSize * row + this.cellSize / 2 + 1, this.cellNumbers[row][col].toString(), 'black', 'black', '1', 'bold ' + (this.cellSize * 0.55) + 'px sans-serif', 'middle', 'middle');
                        if (this.cellNumbers[row][col] > 999 || this.cellNumbers[row][col] < -99) {
                            ttag.setAttribute('textLength', Math.floor(this.cellSize * 0.8).toString());
                            ttag.setAttribute('lengthAdjust', 'spacingAndGlyphs');
                        }
                        ttag.style.touchAction = 'none';
                        ttag.style.pointerEvents = 'none';
                        ttag.style.userSelect = 'none';
                        this.cellNumberTags[row][col] = ttag;
                        this.textRootTag.appendChild(ttag);
                    }
                }
            };
            ChartGroup.prototype.PaintCell = function (px, py) {
                var rootXY = this.svgCanvas.GetGroupPositionValue(this.groupTag);
                var x = px - rootXY.x;
                var y = py - rootXY.y;
                var col = Math.floor(x / this.cellSize);
                var row = Math.floor(y / this.cellSize);
                var color = upred.ui.mathModule.curSelectedColor;
                if (!this.cellColors[row][col]) {
                    this.cellColors[row][col] = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
                    this.cellColors[row][col].setAttribute('x', (this.cellSize * col + 1).toString());
                    this.cellColors[row][col].setAttribute('y', (this.cellSize * row + 1).toString());
                    this.cellColors[row][col].setAttribute('width', (this.cellSize - 2).toString());
                    this.cellColors[row][col].setAttribute('height', (this.cellSize - 2).toString());
                    this.cellColors[row][col].style.pointerEvents = 'none';
                    this.colorsBackTag.appendChild(this.cellColors[row][col]);
                    this.cellColors[row][col].setAttribute('style', 'fill:' + color + '; touch-action: none;');
                }
                else {
                    this.colorsBackTag.removeChild(this.cellColors[row][col]);
                    this.cellColors[row][col] = null;
                }
            };
            ChartGroup.prototype.CheckCell = function (px, py) {
                var rootXY = this.svgCanvas.GetGroupPositionValue(this.groupTag);
                var x = px - rootXY.x;
                var y = py - rootXY.y;
                var col = Math.floor(x / this.cellSize);
                var row = Math.floor(y / this.cellSize);
                var color = upred.ui.mathModule.curSelectedColor;
                if (this.cellMarks[row][col]) {
                    this.marksBackTag.removeChild(this.cellMarks[row][col]);
                    this.cellMarks[row][col] = null;
                }
                else {
                    this.cellMarks[row][col] = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
                    this.cellMarks[row][col].setAttribute('cx', (this.cellSize * col + this.cellSize / 2).toString());
                    this.cellMarks[row][col].setAttribute('cy', (this.cellSize * row + this.cellSize / 2).toString());
                    this.cellMarks[row][col].setAttribute('r', (+this.cellSize / 2 * 0.8).toString());
                    this.cellMarks[row][col].setAttribute('style', 'fill:none; stroke:red; stroke-width:3px; touch-action: none;');
                    this.cellMarks[row][col].style.pointerEvents = 'none';
                    this.marksBackTag.appendChild(this.cellMarks[row][col]);
                }
            };
            ChartGroup.prototype.CopyArray = function (target, source, tagParent) {
                for (var i = 0; i < source.length; i++) {
                    target[i] = [];
                    for (var j = 0; j < source[i].length; j++) {
                        if (tagParent && source[i][j]) {
                            target[i][j] = source[i][j].cloneNode(true);
                            tagParent.appendChild(target[i][j]);
                        }
                        else {
            ...
            ChartGroup.prototype.Duplicate = function () {
                var group = new ChartGroup(this.svgCanvas);
                group.highlighter = this.highlighter;
                group.deleteHandler = this.deleteHandler;
                group.duplicateHandler = this.duplicateHandler;
                group.rotateHandler = this.rotateHandler;
                group.colorChanger = this.colorChanger;
                group.cellSize = this.cellSize;
                group.numStart = this.numStart;
                group.direction = this.direction;
                group.numAdd = this.numAdd;
                group.numMultiple = this.numMultiple;
                group.MakeTable(this.cellNumbers[0].length, this.cellNumbers.length);
                this.CopyArray(group.cellColors, this.cellColors, group.colorsBackTag);
                this.CopyArray(group.cellMarks, this.cellMarks, group.marksBackTag);
                return group;
            };
            ChartGroup.prototype.SetColor = function (r, g, b) {
                this.tableRootTag.firstChild.style.fill = this.MakeRGB(r, g, b, 1);
            };
            ChartGroup.prototype.MakeRGB = function (r, g, b, m) {
                r *= m;
                if (r < 0)
                    r = 0;
                if (r > 255)
        ...
        var NumberChart = (function () {
            function NumberChart() {
                ...
                this.customGrid = {
                    ...
                    oldPX: 0, oldPY: 0,
                    unitW: 1, unitH: 1,
                    direction: NumberDirection.FROM_TOPLEFT_TO_RIGHT
                };
            }
            NumberChart.prototype.MakeRGB = function (r, g, b, m) {
                r *= m;
                if (r < 0)
                    r = 0;
                if (r > 255)
            ...
            NumberChart.prototype.HexToRGB = function (hex) {
                if (hex[0] == '#') {
            ...
            NumberChart.prototype.ChangeColor = function (item, colrstr) {
                if (this.paintMode == 0) {
            ...
            NumberChart.prototype.ChangeColorHandler = function (colorstr) {
                this.curSelectedColor = colorstr;
            };
            NumberChart.prototype.MakePaletteButton = function (imgsrc) {
            ...
            NumberChart.prototype.InitPalette = function () {
                this.svgCanvas = new upred.ui.SVGCanvas();
                this.paletteTag = document.getElementById('UI_PALETTE');
                var chart100bt = this.MakePaletteButton('./asset/icon_grid100.png');
                chart100bt.addEventListener('pointerdown', this.OnDragStartNew100.bind(this));
                var chartCustombt = this.MakePaletteButton('./asset/icon_gridcustom.png');
                chartCustombt.addEventListener('pointerdown', this.OnClickCustomGrid.bind(this));
                this.buttonPaint = this.MakePaletteButton('./asset/icon_paint.png');
                this.buttonPaint.addEventListener('pointerdown', this.OnClickPaint.bind(this));
                this.buttonChecker = this.MakePaletteButton('./asset/icon_checker.png');
                this.buttonChecker.addEventListener('pointerdown', this.OnClickChecker.bind(this));
            ...
            NumberChart.prototype.OnClickCustomGrid = function () {
                this.ResetWorkMode();
                document.getElementById('CUSTOM_MAKE_DLG').style.display = 'block';
            };
            NumberChart.prototype.OnClickPaint = function () {
                if (this.paintMode != 1) {
                    this.paintMode = 1;
                    this.buttonPaint.className = 'paletteButtonSelectedClass';
                    this.buttonChecker.className = 'paletteButtonClass';
                }
                else {
                    this.paintMode = 0;
                    this.buttonPaint.className = 'paletteButtonClass';
                    this.buttonChecker.className = 'paletteButtonClass';
                }
            };
            NumberChart.prototype.OnClickChecker = function () {
                if (this.paintMode != 2) {
                    this.paintMode = 2;
                    this.buttonChecker.className = 'paletteButtonSelectedClass';
                    this.buttonPaint.className = 'paletteButtonClass';
                }
                else {
                    this.paintMode = 0;
            ...
            NumberChart.prototype.ResetWorkMode = function () {
                this.paintMode = 0;
                this.buttonPaint.className = 'paletteButtonClass';
                this.buttonChecker.className = 'paletteButtonClass';
            };
            NumberChart.prototype.OnDragStartNew100 = function (e) {
                var _a;
                if (!e.currentTarget)
                    return;
                ...
                var tgroup = new ChartGroup(this.svgCanvas);
                tgroup.MakeTable(10, 10);
                this.AssignHandlersAndShow(tgroup, this.xyOrg.x, this.xyOrg.y);
            ...
            NumberChart.prototype.OnDragStartMove = function (e) {
                var _a;
                var recttag = e.target;
                if (!recttag || !recttag.parentNode || !recttag.parentNode.parentNode) {
                    return;
                }
                this.curDraggingFigure = recttag.parentNode.parentNode;
                var tgroup = null;
                for (var i = 0; i < this.charts.length; i++) {
                    if (this.charts[i].groupTag == this.curDraggingFigure) {
                        tgroup = this.charts[i];
                        break;
                    }
                }
                if (tgroup == null) {
            ...
            NumberChart.prototype.OnDragging = function (e) {
            ...
            NumberChart.prototype.AddUseTag = function (href, group) {
            ...
            NumberChart.prototype.OnDragEnd = function (e) {
                var _a, _b, _c;
                if (this.dragMode == DragMode.MOVE || this.dragMode == DragMode.NEW) {
                    var dx = this.xyNow.x - this.xyBefore.x;
                    var dy = this.xyNow.y - this.xyBefore.y;
                    if (this.curDraggingFigure != null) {
                        ...
                        if (tg) {
                            (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.UnselectAll();
                            var pPos = this.paletteTag.getBoundingClientRect();
                            if (this.dragMode == DragMode.MOVE && e.pageX < pPos.right && e.pageY > pPos.top) {
                                this.svgCanvas.RemoveGroup(this.curDraggingFigure);
                                this.charts.splice(this.charts.indexOf(tg), 1);
                                this.ResetWorkMode();
                            }
                            else {
                                if (dx * dx + dy * dy < 32) {
                                    if (this.paintMode == 1) {
                                        tg.PaintCell(e.pageX, e.pageY);
                                    }
                                    else if (this.paintMode == 2) {
                                        tg.CheckCell(e.pageX, e.pageY);
                                    }
                                    else {
                                        (_b = upred.ui.uiModule) === null || _b === void 0 ? void 0 : _b.SelectItemOnlyOne(tg);
                                    }
                                    if (this.dragMode == DragMode.NEW) {
                                        var sz = tg.groupTag.getBoundingClientRect();
                                        var newx = Math.floor((window.innerWidth - sz.width) / 2);
                                        var newy = Math.floor((window.innerHeight - sz.height) / 2);
                                        this.svgCanvas.SetGroupPositionValue(tg.groupTag, newx, newy);
                                        (_c = upred.ui.uiModule) === null || _c === void 0 ? void 0 : _c.SelectItemOnlyOne(tg);
                                    }
                                }
                                else {
                                    this.ResetWorkMode();
            ...
            NumberChart.prototype.AssignHandlersAndShow = function (group, x, y) {
                ...
                if (this.charts.indexOf(group) < 0)
                    this.charts.push(group);
            };
            NumberChart.prototype.ShowHighlighted = function (group, flag) {
                if (group) {
                    if (flag) {
            ...
            NumberChart.prototype.SetSelectionBy = function (x, y, w, h) {
                var _a;
                for (var i = 0; i < this.charts.length; i++) {
                    var ar = this.charts[i].groupTag.getBoundingClientRect();
                    if (!(x > ar.right ||
                        x + w < ar.left ||
                        y > ar.bottom ||
                        y + h < ar.top)) {
                        (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.SelectItemAdd(this.charts[i]);
                    }
                }
            };
            NumberChart.prototype.Rotate = function (group, degree) {
            ...
            NumberChart.prototype.Delete = function (targetGroup) {
                if (targetGroup != null && this.charts.indexOf(targetGroup) >= 0) {
                    this.svgCanvas.RemoveGroup(targetGroup.groupTag);
                    this.charts.splice(this.charts.indexOf(targetGroup), 1);
                }
            };
            NumberChart.prototype.Duplicate = function (targetGroup) {
                var _a, _b;
                if (targetGroup != null) {
                    var xy = this.svgCanvas.GetGroupPositionValue(targetGroup.groupTag);
                    var groupB = targetGroup.Duplicate();
                    this.charts.push(groupB);
                    this.svgCanvas.SetGroupPositionValue(groupB.groupTag, xy.x + this.CELLSize + 2, xy.y + this.CELLSize + 2);
            ...
            NumberChart.prototype.RestoreInitialState = function () {
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:400;');
                for (var i = 0; i < this.charts.length; i++) {
                    this.svgCanvas.RemoveGroup(this.charts[i].groupTag);
                }
                this.charts = [];
            };
            NumberChart.prototype.EnableInteraction = function () {
                this.svgCanvas.MoveToFront();
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:750;');
            };
            NumberChart.prototype.DisableInteraction = function () {
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:400;');
            };
            NumberChart.prototype.InitCustomDialog = function () {
                var table = document.getElementById('CELL_SELECT_TABLE');
                for (var i = 0; i < 16; i++) {
                    var tr = document.createElement('tr');
                    table === null || table === void 0 ? void 0 : table.appendChild(tr);
                    for (var j = 0; j < 16; j++) {
                        var td = document.createElement('td');
                        td.className = 'cellSelectorTd';
                        td.CUSTOM_ROW = i;
                        td.CUSTOM_COL = j;
                        td.id = 'CUSTOM_CELL_INDI_' + j + 'BY' + i;
                        td.innerHTML = '&nbsp;';
                        td.onpointerdown = this.OnCustomCellDown.bind(this);
                        tr.appendChild(td);
                    }
                }
                window.addEventListener('pointermove', this.OnCustomCellMove.bind(this));
                window.addEventListener('pointerup', this.OnCustomCellUp.bind(this));
                document.getElementById('CMGD_COLUMN').oninput = this.OnChangeCellSize.bind(this);
                document.getElementById('CMGD_ROW').oninput = this.OnChangeCellSize.bind(this);
                document.getElementById('CUSTOMMAKE_OK').onclick = this.OnClickOKCustomMake.bind(this);
                document.getElementById('CUSTOMMAKE_CANCEL').onclick = this.OnClickCancelCustomMake.bind(this);
            };
            NumberChart.prototype.OnClickOKCustomMake = function () {
                var _a;
                if (this.customGrid.col <= 0 || this.customGrid.row <= 0) {
                    alert('크기가 올바르게 설정되지 않았습니다');
                    return;
                }
                document.getElementById('CUSTOM_MAKE_DLG').style.display = 'none';
                var tgroup = new ChartGroup(this.svgCanvas);
                tgroup.numStart = parseInt(document.getElementById('CMGD_STARTNO').value);
                tgroup.numMultiple = parseInt(document.getElementById('CMGD_MULTI').value);
                tgroup.numAdd = parseInt(document.getElementById('CMGD_ADD').value);
                var el = document.getElementsByName('CMGD_DIRECTION');
                ...
                tgroup.MakeTable(this.customGrid.col, this.customGrid.row);
                this.AssignHandlersAndShow(tgroup, 150, 50);
                (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.SelectItemOnlyOne(tgroup);
                this.svgCanvas.GetTag().appendChild(tgroup.groupTag);
            };
            NumberChart.prototype.OnClickCancelCustomMake = function () {
                document.getElementById('CUSTOM_MAKE_DLG').style.display = 'none';
            };
            NumberChart.prototype.OnCustomCellDown = function (e) {
                var td = e.currentTarget;
                this.customGrid.oldPX = e.pageX;
                this.customGrid.oldPY = e.pageY;
                var sz = document.getElementById('CELL_SELECT_TABLE').getBoundingClientRect();
                this.customGrid.unitW = sz.width / 16;
                this.customGrid.unitH = sz.height / 16;
                this.customGrid.ex = this.customGrid.sx = td.CUSTOM_COL;
                this.customGrid.ey = this.customGrid.sy = td.CUSTOM_ROW;
                this.customGrid.nowDragging = true;
                this.UpdateCustomGrid();
            };
            NumberChart.prototype.OnCustomCellUp = function (e) {
                this.customGrid.nowDragging = false;
            };
            NumberChart.prototype.OnCustomCellMove = function (e) {
                if (!this.customGrid.nowDragging)
                    return;
                var dx = e.pageX - this.customGrid.oldPX;
                var dy = e.pageY - this.customGrid.oldPY;
                var dxN = Math.floor(dx / this.customGrid.unitW);
                var dyN = Math.floor(dy / this.customGrid.unitH);
                this.customGrid.ex = this.customGrid.sx + dxN;
                this.customGrid.ey = this.customGrid.sy + dyN;
                this.UpdateCustomGrid();
            };
            NumberChart.prototype.OnChangeCellSize = function () {
                this.customGrid.col = document.getElementById('CMGD_COLUMN').value;
                this.customGrid.row = document.getElementById('CMGD_ROW').value;
                if (this.customGrid.col <= 0) {
                    this.customGrid.col = 1;
                    document.getElementById('CMGD_COLUMN').value = '1';
                }
                if (this.customGrid.row <= 0) {
                    this.customGrid.row = 1;
                    document.getElementById('CMGD_ROW').value = '1';
                }
                this.customGrid.sx = this.customGrid.sy = 0;
                this.customGrid.ex = this.customGrid.sx + this.customGrid.col - 1;
                this.customGrid.ey = this.customGrid.sy + this.customGrid.row - 1;
                this.UpdateCustomGrid(true);
            };
            NumberChart.prototype.UpdateCustomGrid = function (donotUpdateInput) {
                if (donotUpdateInput === void 0) { donotUpdateInput = false; }
                var SX = Math.min(this.customGrid.sx, this.customGrid.ex);
                var EX = Math.max(this.customGrid.sx, this.customGrid.ex);
                var SY = Math.min(this.customGrid.sy, this.customGrid.ey);
                var EY = Math.max(this.customGrid.sy, this.customGrid.ey);
                for (var i = 0; i < 16; i++) {
                    for (var j = 0; j < 16; j++) {
                        var td = document.getElementById('CUSTOM_CELL_INDI_' + j + 'BY' + i);
                        if (j >= SX && j <= EX && i >= SY && i <= EY) {
                            td.style.background = '#7FB3D5';
                            td.style.border = '#2980B9 1px solid';
                        }
                        else {
                            td.style.background = 'none';
                            td.style.border = '#AAAAAA 1px solid';
                        }
                    }
                }
                this.customGrid.col = EX - SX + 1;
                this.customGrid.row = EY - SY + 1;
                if (!donotUpdateInput) {
                    document.getElementById('CMGD_COLUMN').value = this.customGrid.col;
                    document.getElementById('CMGD_ROW').value = this.customGrid.row;
                }
            };
            NumberChart.prototype.AdjustCellSize = function (cgroup, colCount, rowCount) {
                ...
                var w = 0, h = 0;
                if (RW > RH) {
                    w = Math.floor(RW * 0.45 / colCount);
                    h = Math.floor(RH * 0.8 / rowCount);
                }
                else {
                    w = Math.floor(RW * 0.8 / colCount);
                    h = Math.floor(RH * 0.45 / rowCount);
                }
                cgroup.cellSize = Math.min(w, h);
            };
            NumberChart.prototype.Init = function () {
                var _a;
        ...
        math.NumberChart = NumberChart;
    })(math = upred.math || (upred.math = {}));
})(upred || (upred = {}));
//# sourceMappingURL=numberchart.js.map