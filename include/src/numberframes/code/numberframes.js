...
(function (upred) {
    ...
    (function (math) {
        ...
        (function (GroupType) {
            ...
            GroupType[GroupType["CHIP"] = 1] = "CHIP";
            GroupType[GroupType["TABLE5"] = 2] = "TABLE5";
            GroupType[GroupType["TABLE10"] = 3] = "TABLE10";
        })(GroupType || (GroupType = {}));
        var DragMode;
        (function (DragMode) {
        ...
        var NFGroup = (function () {
            function NFGroup(canvas) {
            ...
            NFGroup.prototype.CanBeInserted = function (svgCanvas, tg, unitSize) {
                if (this.groupType == GroupType.CHIP)
                    return -1;
                var xy1 = svgCanvas.GetGroupPositionValue(tg.groupTag);
                var xy2 = svgCanvas.GetGroupPositionValue(this.groupTag);
                var x1 = xy1.x;
                var y1 = xy1.y;
                var x2 = xy1.x + unitSize;
                var y2 = xy1.y + unitSize;
                var MX = Math.floor((x1 + x2) / 2);
                var MY = Math.floor((y1 + y2) / 2);
                var inbound = (unitSize / 2) * (unitSize / 2);
                var my = Math.floor(xy2.y + unitSize / 2 + 3);
                for (var i = 0, sx = xy2.x; i < 5; i++, sx += unitSize) {
                    if (this.innerChips[i])
                        continue;
                    var ex = sx + unitSize;
                    var mx = Math.floor((sx + ex) / 2);
                    var distance = (MX - mx) * (MX - mx) + (MY - my) * (MY - my);
                    if (distance < inbound) {
                        this.hoverTag.setAttribute('x', (sx - xy2.x).toString());
                        this.hoverTag.setAttribute('y', '2');
                        this.hoverTag.setAttribute('style', 'fill:#2980B9; opacity:0.3; stroke:none;');
                        this.groupTag.appendChild(this.hoverTag);
                        return i;
                    }
                }
                if (this.groupType == GroupType.TABLE10) {
                    my += unitSize;
                    for (var i = 5, sx = xy2.x; i < 10; i++, sx += unitSize) {
                        if (this.innerChips[i])
                            continue;
                        var ex = sx + unitSize;
                        var mx = Math.floor((sx + ex) / 2);
                        ...
                        if (distance < inbound) {
                            this.hoverTag.setAttribute('x', (sx - xy2.x).toString());
                            this.hoverTag.setAttribute('y', (unitSize + 2).toString());
                            this.hoverTag.setAttribute('style', 'fill:#2980B9; opacity:0.3; stroke:none;');
                            this.groupTag.appendChild(this.hoverTag);
                            return i;
                        }
            ...
            NFGroup.prototype.InsertAny = function (tg, unitSize, gap) {
                var start = 0;
                var started = false;
                for (var i = 0; i < this.innerChips.length; i++) {
                    if (this.innerChips[i] != null) {
                        started = true;
                        continue;
                    }
                    if (this.innerChips[i] == null) {
                ...
                if (start >= this.innerChips.length)
                    start = 0;
                for (var i = start; i < this.innerChips.length; i++) {
                    if (this.innerChips[i] == null) {
                        var gTag = tg.groupTag;
                        this.groupTag.appendChild(gTag);
                        var x = i % 5;
                        var y = Math.floor(i / 5);
                        gTag.setAttribute('transform', 'translate(' + (x * unitSize + gap + 1).toString() + ',' + (y * unitSize + gap + 2).toString() + ')');
                        this.innerChips[i] = gTag;
                        return;
                    }
                }
            };
            NFGroup.prototype.Insert = function (svgCanvas, tg, unitSize, gap) {
                var index = this.CanBeInserted(svgCanvas, tg, unitSize);
                if (index >= 0) {
                    var gTag = tg.groupTag;
                    this.groupTag.appendChild(gTag);
                    var x = index % 5;
                    ...
                    gTag.setAttribute('transform', 'translate(' + (x * unitSize + gap + 1).toString() + ',' + (y * unitSize + gap + 2).toString() + ')');
                    this.innerChips[index] = gTag;
                }
                return index;
            };
            NFGroup.prototype.HideHovering = function () {
                if (this.hoverTag && this.groupTag.contains(this.hoverTag)) {
                    this.groupTag.removeChild(this.hoverTag);
                }
            };
            NFGroup.prototype.MakeCells = function (unitSize) {
                var n = 0;
                if (this.groupType == GroupType.TABLE5)
                    n = 5;
                else if (this.groupType == GroupType.TABLE10)
                    n = 10;
                else
                    return;
                this.MakeHoveringTag(unitSize);
                for (var i = 0; i < n; i++) {
                    this.innerChips[i] = null;
                }
            };
            NFGroup.prototype.MakeHoveringTag = function (unitSize) {
                this.hoverTag = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
                this.hoverTag.setAttribute('width', (unitSize));
                this.hoverTag.setAttribute('height', (unitSize));
            };
            NFGroup.prototype.SetColor = function (r, g, b) {
                var styletext = 'fill:' +
            ...
            NFGroup.prototype.MakeRGB = function (r, g, b, m) {
                r *= m;
                if (r < 0)
                    r = 0;
                if (r > 255)
        ...
        var NumberFrames = (function () {
            ...
            NumberFrames.prototype.MakeRGB = function (r, g, b, m) {
                r *= m;
                if (r < 0)
                    r = 0;
                if (r > 255)
            ...
            NumberFrames.prototype.MakeFigure = function (ids) {
                var symbol = new upred.ui.SVGSymbol(ids);
                symbol.AddCircle(this.NSize / 2, this.NSize / 2, this.NSize / 2 - 2);
                symbol.SetSize(this.NSize.toString(), this.NSize.toString());
                return symbol;
            };
            NumberFrames.prototype.MakeTable = function (ids, colN, rowN) {
                var symbol = new upred.ui.SVGSymbol(ids);
                var w = colN * (this.NSize + this.GAP);
                var h = rowN * (this.NSize + this.GAP);
                symbol.AddRectangle(2, 2, w, h);
                for (var y = 1; y < rowN; y++) {
                    var yy = y * (this.NSize + this.GAP) + 2;
                    symbol.AddLine(1, yy, w + 1, yy, 1, 'butt', 'bevel', 'black');
                }
                for (var x = 1; x < colN; x++) {
                    var xx = x * (this.NSize + this.GAP);
                    symbol.AddLine(xx, 1, xx, h + 1, 1, 'butt', 'bevel', 'black');
                }
                symbol.AddLine(0, 0, w + 1, 0, 2, 'butt', 'bevel', 'black');
                symbol.AddLine(0, 0, 0, h + 1, 2, 'butt', 'bevel', 'black');
                symbol.AddLine(w + 1, 0, w + 1, h + 1, 2, 'butt', 'bevel', 'black');
                symbol.AddLine(0, h + 1, w + 1, h + 1, 2, 'butt', 'bevel', 'black');
                symbol.SetSize((w + 1).toString(), (h + 1).toString());
                return symbol;
            };
            NumberFrames.prototype.HexToRGB = function (hex) {
                if (hex[0] == '#') {
            ...
            NumberFrames.prototype.ChangeColor = function (item, colrstr) {
                var color = this.HexToRGB(colrstr);
                if (item.SetColor) {
                    item.SetColor(color.r, color.g, color.b);
                }
            };
            NumberFrames.prototype.InitPalette = function () {
                this.svgCanvas = new upred.ui.SVGCanvas();
                this.paletteTag = document.getElementById('UI_PALETTE');
                this.MakeFigure('ChipFigure');
                this.MakeTable('TableFigure5', 5, 1);
                this.MakeTable('TableFigure10', 5, 2);
                var table5 = new upred.ui.SVGUnit();
                table5.AddSymbol('TableFigure5');
                table5.GetTag().style.display = 'inline-block';
                table5.GetTag().style.margin = '8px';
                table5.GetTag().style.touchAction = 'none';
                table5.GetTag().setAttribute("pointer-events", "visiblePainted");
                table5.GetTag().firstChild.style.fill = 'white';
                this.paletteTag.appendChild(table5.GetTag());
                table5.GetTag().addEventListener('pointerdown', this.OnDragStartNew.bind(this));
                table5.SetViewBox(0, 0, (this.NSize + this.GAP) * 5 + 3, (this.NSize + this.GAP) + 3);
                table5.SetSize(((this.NSize + this.GAP) * 5) / 2, (this.NSize + this.GAP) / 2);
                var table10 = new upred.ui.SVGUnit();
                table10.AddSymbol('TableFigure10');
                table10.GetTag().style.display = 'inline-block';
                table10.GetTag().style.margin = '8px';
                table10.GetTag().style.touchAction = 'none';
                table10.GetTag().setAttribute("pointer-events", "visiblePainted");
                table10.GetTag().firstChild.style.fill = 'white';
                this.paletteTag.appendChild(table10.GetTag());
                table10.GetTag().addEventListener('pointerdown', this.OnDragStartNew.bind(this));
                table10.SetViewBox(0, 0, (this.NSize + this.GAP) * 5 + 3, (this.NSize + this.GAP) * 2 + 3);
                table10.SetSize(((this.NSize + this.GAP) * 5) / 2, (this.NSize + this.GAP));
                var chip1 = new upred.ui.SVGUnit();
                chip1.AddSymbol('ChipFigure');
                chip1.GetTag().style.display = 'inline-block';
                chip1.GetTag().style.margin = '8px';
                chip1.GetTag().style.touchAction = 'none';
                chip1.GetTag().style.fill = this.MakeRGB(this.presetColors[3], this.presetColors[4], this.presetColors[5], 1);
                chip1.GetTag().style.stroke = this.MakeRGB(this.presetColors[3], this.presetColors[4], this.presetColors[5], 0.5);
                chip1.GetTag().style.strokeWidth = '1';
                chip1.GetTag().setAttribute("pointer-events", "visiblePainted");
                this.paletteTag.appendChild(chip1.GetTag());
                chip1.GetTag().addEventListener('pointerdown', this.OnDragStartNew.bind(this));
                chip1.AddText(0, 0, "1");
                var chip5 = new upred.ui.SVGUnit();
                for (var i = 0; i < 5; i++) {
                    chip5.AddSymbol('ChipFigure');
                    chip5.subTag[i].setAttribute("x", ((4 - i) * 4).toString());
                }
                chip5.GetTag().style.display = 'inline-block';
                chip5.GetTag().style.margin = '8px';
                chip5.GetTag().style.touchAction = 'none';
                chip5.GetTag().style.fill = this.MakeRGB(this.presetColors[3], this.presetColors[4], this.presetColors[5], 1);
                chip5.GetTag().style.stroke = this.MakeRGB(this.presetColors[3], this.presetColors[4], this.presetColors[5], 0.5);
                chip5.GetTag().style.strokeWidth = '1';
                chip5.GetTag().setAttribute("pointer-events", "visiblePainted");
                this.paletteTag.appendChild(chip5.GetTag());
                chip5.GetTag().addEventListener('pointerdown', this.OnDragStartNew.bind(this));
                chip5.SetSize(this.NSize + 4 * 4, this.NSize);
                chip5.AddText(0, 0, "5");
                var chip10 = new upred.ui.SVGUnit();
                for (var i = 0; i < 10; i++) {
                    chip10.AddSymbol('ChipFigure');
                    chip10.subTag[i].setAttribute("x", ((9 - i) * 4).toString());
                }
                chip10.GetTag().style.display = 'inline-block';
                chip10.GetTag().style.margin = '8px';
                chip10.GetTag().style.touchAction = 'none';
                chip10.GetTag().style.fill = this.MakeRGB(this.presetColors[3], this.presetColors[4], this.presetColors[5], 1);
                chip10.GetTag().style.stroke = this.MakeRGB(this.presetColors[3], this.presetColors[4], this.presetColors[5], 0.5);
                chip10.GetTag().style.strokeWidth = '1';
                chip10.GetTag().setAttribute("pointer-events", "visiblePainted");
                this.paletteTag.appendChild(chip10.GetTag());
                chip10.GetTag().addEventListener('pointerdown', this.OnDragStartNew.bind(this));
                chip10.SetSize(this.NSize + 9 * 4, this.NSize);
                chip10.AddText(0, 0, "10");
            ...
            NumberFrames.prototype.OnDragStartNew = function (e) {
                var _a;
                if (!e.currentTarget)
                    return;
                ...
                if (tgroup.groupTag.firstChild.getAttribute('href') != '#ChipFigure') {
                    tgroup.SetColor(255, 255, 255);
                }
                else {
            ...
            NumberFrames.prototype.OnDragStartMove = function (e) {
                var _a, _b;
                this.originalFigure = undefined;
                var usetag = e.target;
                if (!usetag)
                ...
                for (var i = 0; i < this.npGroups.length; i++) {
                    if (this.npGroups[i].groupTag == usetag.parentNode) {
                ...
                var href = usetag.getAttribute('href');
                if (href == '#ChipFigure') {
                    for (var i = 0; i < this.npGroups.length; i++) {
                        if (this.npGroups[i] == tgroup)
                            continue;
                        var innerIndex = this.npGroups[i].innerChips.indexOf(tgroup.groupTag);
                        if (innerIndex >= 0) {
                            var xyparent = this.svgCanvas.GetGroupPositionValue(this.npGroups[i].groupTag);
                            var innerY = Math.floor(innerIndex / 5);
                            var innerX = innerIndex % 5;
                            var xp = xyparent.x + innerX * (this.NSize + this.GAP) + 1;
                            var yp = xyparent.y + innerY * (this.NSize + this.GAP) + 2;
                            this.svgCanvas.RestoreGroup(tgroup.groupTag, xp, yp, 0);
                            this.npGroups[i].innerChips[innerIndex] = null;
                            break;
                        }
                    }
                }
            ...
            NumberFrames.prototype.OnDragging = function (e) {
                if (this.dragMode == DragMode.MOVE || this.dragMode == DragMode.NEW) {
                    ...
                    if (tg) {
                        if (tg.groupType == GroupType.CHIP) {
                            for (var i = 0; i < this.npGroups.length; i++) {
                                var container = this.npGroups[i];
                                var index = container.CanBeInserted(this.svgCanvas, tg, this.NSize + this.GAP);
                                if (index >= 0) {
                                    break;
                                }
                                else {
                                    container.HideHovering();
                                }
                            }
                        }
                    }
            ...
            NumberFrames.prototype.OnDragEnd = function (e) {
                var _a, _b;
                if (this.dragMode == DragMode.MOVE || this.dragMode == DragMode.NEW) {
                    var dx = this.xyNow.x - this.xyBefore.x;
                    var dy = this.xyNow.y - this.xyBefore.y;
                    if (this.curDraggingFigure != null) {
                        ...
                        if (tg) {
                            ...
                            if (pos.right < pPos.right && pos.top > pPos.top) {
                                ...
                                var restChips = [];
                                if (this.dragMode == DragMode.NEW && tg.groupType == GroupType.CHIP) {
                                    ...
                                    for (var i = 1; i < count; i++) {
                                        ...
                                        newGroup.groupType = GroupType.CHIP;
                                ...
                                if (tg.groupType == GroupType.CHIP) {
                                    for (var i = 0; i < this.npGroups.length; i++) {
                                        var container = this.npGroups[i];
                                        var index = container.Insert(this.svgCanvas, tg, this.NSize + this.GAP, this.GAP / 2);
                                        container.HideHovering();
                                        if (index >= 0) {
                                            for (var j = 0; j < restChips.length; j++) {
                                                container.InsertAny(restChips[j], this.NSize + this.GAP, this.GAP / 2);
                                            }
                                            break;
                                        }
                                    }
            ...
            NumberFrames.prototype.RemoveByGroupTag = function (grouptag) {
                var n = -1;
                for (var i = 0; i < this.npGroups.length; i++) {
            ...
            NumberFrames.prototype.MakeNewGroupBase = function (x, y) {
                var group = new NFGroup(this.svgCanvas);
            ...
            NumberFrames.prototype.MakeNewGroup = function (x, y, oldParent) {
                var group = this.MakeNewGroupBase(x, y);
                if (oldParent) {
                    var isChip = false;
                    while (oldParent.firstChild) {
                        if (oldParent.firstChild.nodeName == 'text' || oldParent.firstChild.nodeName == 'TEXT') {
                            isChip = true;
                            break;
                        }
                        group.groupTag.appendChild(oldParent.firstChild);
                    }
                    group.groupType = GroupType.CHIP;
                    if (!isChip) {
                        var href = group.groupTag.firstChild.getAttribute('href');
                        if (href == '#TableFigure5')
                            group.groupType = GroupType.TABLE5;
                        else if (href == '#TableFigure10')
                            group.groupType = GroupType.TABLE10;
                        group.MakeCells(this.NSize + this.GAP);
                    }
                }
                else {
            ...
            NumberFrames.prototype.ShowHighlighted = function (group, flag) {
                if (group) {
                    if (flag) {
            ...
            NumberFrames.prototype.SetSelectionBy = function (x, y, w, h) {
                var _a;
                for (var i = 0; i < this.npGroups.length; i++) {
            ...
            NumberFrames.prototype.Rotate = function (group, degree) {
            ...
            NumberFrames.prototype.Delete = function (targetGroup) {
                if (targetGroup != null && this.npGroups.indexOf(targetGroup) >= 0) {
                    if (targetGroup.groupType == GroupType.CHIP) {
                        var inner = false;
                        for (var i = 0; i < this.npGroups.length; i++) {
                            var innerIndex = this.npGroups[i].innerChips.indexOf(targetGroup.groupTag);
                            if (innerIndex >= 0) {
                                var xyparent = this.svgCanvas.GetGroupPositionValue(this.npGroups[i].groupTag);
                                var innerY = Math.floor(innerIndex / 5);
                                var innerX = innerIndex % 5;
                                var xp = xyparent.x + innerX * (this.NSize + this.GAP) + 1;
                                var yp = xyparent.y + innerY * (this.NSize + this.GAP) + 2;
                                this.svgCanvas.RestoreGroup(targetGroup.groupTag, xp, yp, 0);
                                this.npGroups[i].innerChips[innerIndex] = null;
                                inner = true;
                                break;
                            }
            ...
            NumberFrames.prototype.Duplicate = function (targetGroup) {
                var _a, _b;
                if (targetGroup != null) {
                    ...
                    for (var i = 0; i < targetGroup.groupTag.children.length; i++) {
                        if (targetGroup.groupTag.children[i].nodeName == 'g' || targetGroup.groupTag.children[i].nodeName == 'G') {
                            continue;
                        }
                        else {
                            groupB.groupTag.appendChild(targetGroup.groupTag.children[i].cloneNode(true));
                        }
                    }
                    var insideCount = 0;
                    if (targetGroup.groupType == GroupType.TABLE10) {
                        insideCount = 10;
                        groupB.MakeHoveringTag(this.NSize + this.GAP);
                    }
                    else if (targetGroup.groupType == GroupType.TABLE5) {
                        insideCount = 5;
                        groupB.MakeHoveringTag(this.NSize + this.GAP);
                    }
                    for (var i = 0; i < insideCount; i++) {
                        var inside = targetGroup.innerChips[i];
                        if (inside != null) {
                            var x = i % 5;
                            var y = Math.floor(i / 5);
                            var inGroup = this.MakeNewGroupBase((x * (this.NSize + this.GAP) + this.GAP / 2 + 1), (y * (this.NSize + this.GAP) + this.GAP / 2 + 2));
                            inGroup.groupTag.appendChild(inside.firstChild.cloneNode(true));
                            groupB.innerChips[i] = inGroup.groupTag;
                            groupB.groupTag.appendChild(inGroup.groupTag);
                            inGroup.groupType = GroupType.CHIP;
                        }
                        else {
                            groupB.innerChips[i] = null;
                        }
                    }
                    this.svgCanvas.SetGroupPositionValue(groupB.groupTag, xy.x + this.NSize / 2, xy.y + this.NSize / 2);
            ...
            NumberFrames.prototype.RestoreInitialState = function () {
            ...
            NumberFrames.prototype.EnableInteraction = function () {
                this.svgCanvas.MoveToFront();
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:750;');
            };
            NumberFrames.prototype.DisableInteraction = function () {
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:400;');
            };
            NumberFrames.prototype.Init = function () {
                var _a;
        ...
        math.NumberFrames = NumberFrames;
    })(math = upred.math || (upred.math = {}));
})(upred || (upred = {}));
//# sourceMappingURL=numberframes.js.map