...
(function (upred) {
    ...
    (function (math) {
        ...
        (function (GroupType) {
            ...
            GroupType[GroupType["CHIPS10"] = 2] = "CHIPS10";
        })(GroupType || (GroupType = {}));
        var DragMode;
        (function (DragMode) {
        ...
        var ChipsGroup = (function () {
            function ChipsGroup(canvas) {
            ...
            ChipsGroup.prototype.SetColor = function (r, g, b) {
                var styletext = 'fill:' +
            ...
            ChipsGroup.prototype.MakeRGB = function (r, g, b, m) {
                r *= m;
                if (r < 0)
                    r = 0;
                if (r > 255)
        ...
        var Chips = (function () {
            ...
            Chips.prototype.MakeRGB = function (r, g, b, m) {
                r *= m;
                if (r < 0)
                    r = 0;
                if (r > 255)
            ...
            Chips.prototype.MakeFigure = function (ids) {
                var symbol = new upred.ui.SVGSymbol(ids);
                symbol.AddCircle(this.NSize / 2, this.NSize / 2 + 4, this.NSize / 2 - 2);
                symbol.subTag[symbol.subTag.length - 1].style.strokeWidth = '1';
                symbol.subTag[symbol.subTag.length - 1].style.filter = 'brightness(50%)';
                symbol.AddCircle(this.NSize / 2, this.NSize / 2, this.NSize / 2 - 2);
                symbol.subTag[symbol.subTag.length - 1].style.strokeWidth = '1';
                symbol.AddPath('M 11 63 A 37 37 0 0 1 63 11 Z', '', '', '0', 'square', 'miter');
                symbol.subTag[symbol.subTag.length - 1].style.filter = 'brightness(105%)';
                symbol.AddCircle(this.NSize / 2, this.NSize / 2, (this.NSize / 2 - 2) * 0.7);
                symbol.subTag[symbol.subTag.length - 1].style.filter = 'brightness(78%)';
                symbol.subTag[symbol.subTag.length - 1].style.strokeWidth = '0';
                symbol.AddPath('M 35 66 A 26 26 0 1 1 66 35 Z', '', '', '0', 'square', 'miter');
                symbol.subTag[symbol.subTag.length - 1].style.filter = 'brightness(82%)';
                symbol.SetSize(this.NSize.toString(), this.NSize.toString());
                return symbol;
            };
            Chips.prototype.HexToRGB = function (hex) {
                if (hex[0] == '#') {
            ...
            Chips.prototype.ChangeColor = function (item, colrstr) {
                var color = this.HexToRGB(colrstr);
                if (item.SetColor) {
                    item.SetColor(color.r, color.g, color.b);
                }
            };
            Chips.prototype.InitPalette = function () {
                this.svgCanvas = new upred.ui.SVGCanvas();
                this.paletteTag = document.getElementById('UI_PALETTE');
                this.MakeFigure('ChipFigure');
                for (var i = 0; i < 5; i++) {
                    var chip_1 = new upred.ui.SVGUnit();
                    chip_1.AddSymbol('ChipFigure');
                    chip_1.subTag[0].COLORINDEXV = i;
                    chip_1.GetTag().style.display = 'inline-block';
                    chip_1.GetTag().style.margin = '8px';
                    chip_1.GetTag().style.touchAction = 'none';
                    chip_1.GetTag().setAttribute("pointer-events", "visiblePainted");
                    this.paletteTag.appendChild(chip_1.GetTag());
                    chip_1.GetTag().addEventListener('pointerdown', this.OnDragStartNew.bind(this));
                    chip_1.GetTag().firstChild.style.fill = this.MakeRGB(this.presetColors[i * 3], this.presetColors[i * 3 + 1], this.presetColors[i * 3 + 2], 1);
                    chip_1.GetTag().firstChild.style.fill = this.MakeRGB(this.presetColors[i * 3], this.presetColors[i * 3 + 1], this.presetColors[i * 3 + 2], 1);
                    chip_1.GetTag().firstChild.style.stroke = this.MakeRGB(this.presetColors[i * 3], this.presetColors[i * 3 + 1], this.presetColors[i * 3 + 2], 0.5);
                    chip_1.GetTag().firstChild.style.strokeWidth = '2';
                    chip_1.SetSize(this.NSize + 2, this.NSize + 3);
                }
                var chip = new upred.ui.SVGUnit();
                for (var i = 0; i < 10; i++) {
                    chip.AddSymbol('ChipFigure');
                    chip.subTag[i].COLORINDEXV = 4 - (i % 5);
                    chip.subTag[i].setAttribute('y', (9 - i) * 5);
                    chip.subTag[i].style.fill = this.MakeRGB(this.presetColors[(4 - (i % 5)) * 3], this.presetColors[((4 - (i % 5)) * 3) + 1], this.presetColors[((4 - (i % 5)) * 3) + 2], 1);
                    chip.subTag[i].style.fill = this.MakeRGB(this.presetColors[(4 - (i % 5)) * 3], this.presetColors[((4 - (i % 5)) * 3) + 1], this.presetColors[((4 - (i % 5)) * 3) + 2], 1);
                    chip.subTag[i].style.stroke = this.MakeRGB(this.presetColors[(4 - (i % 5)) * 3], this.presetColors[((4 - (i % 5)) * 3) + 1], this.presetColors[((4 - (i % 5)) * 3) + 2], 0.5);
                    chip.subTag[i].style.strokeWidth = '2';
                }
                chip.GetTag().style.display = 'inline-block';
                chip.GetTag().style.margin = '8px';
                chip.GetTag().style.touchAction = 'none';
                chip.GetTag().setAttribute("pointer-events", "visiblePainted");
                chip.GetTag().appendChild(this.MakeText('10'));
                this.paletteTag.appendChild(chip.GetTag());
                chip.GetTag().addEventListener('pointerdown', this.OnDragStartNew.bind(this));
                chip.SetSize(this.NSize + 2, this.NSize + 3 + 50);
            ...
            Chips.prototype.OnDragStartNew = function (e) {
                var _a;
                if (!e.currentTarget)
                    return;
                ...
                var COLORINDEXV = this.originalFigure.firstChild.COLORINDEXV;
                var tempParent = this.originalFigure.cloneNode(true);
                tempParent.firstChild.COLORINDEXV = COLORINDEXV;
            ...
            Chips.prototype.OnDragStartMove = function (e) {
                var _a, _b;
                this.originalFigure = undefined;
                var usetag = e.target;
                if (!usetag)
                ...
                var tgroup = null;
                for (var i = 0; i < this.chipsGroups.length; i++) {
                    if (this.chipsGroups[i].groupTag == usetag.parentNode) {
                        tgroup = this.chipsGroups[i];
                        this.curDraggingFigure = this.chipsGroups[i].groupTag;
                        break;
                    }
                }
            ...
            Chips.prototype.OnDragging = function (e) {
                if (this.dragMode == DragMode.MOVE || this.dragMode == DragMode.NEW) {
                    ...
                    this.svgCanvas.SetGroupPositionValue(this.curDraggingFigure, nx, ny);
                    ...
                    var tg = null;
                    for (var i = 0; i < this.chipsGroups.length; i++) {
                        if (this.chipsGroups[i].groupTag == this.curDraggingFigure) {
                            tg = this.chipsGroups[i];
                            break;
                        }
                    }
                    if (tg) {
                        for (var i = 0; i < this.chipsGroups.length; i++) {
                            if (this.chipsGroups[i] == tg)
                                continue;
                            if (this.chipsGroups[i].groupType == GroupType.CHIPS10)
                                continue;
                            var ddxy = this.svgCanvas.GetGroupPositionValue(this.chipsGroups[i].groupTag);
                            if (ny < ddxy.y && ny > ddxy.y - 30 && nx > ddxy.x - 20 && nx < ddxy.x + 20) {
                                this.svgCanvas.SetGroupPositionValue(this.curDraggingFigure, ddxy.x, ddxy.y - 5);
                            }
                        }
                    }
                }
            };
            Chips.prototype.OnDragEnd = function (e) {
                var _a, _b;
                if (this.dragMode == DragMode.MOVE || this.dragMode == DragMode.NEW) {
                    var dx = this.xyNow.x - this.xyBefore.x;
                    var dy = this.xyNow.y - this.xyBefore.y;
                    if (this.curDraggingFigure != null) {
                        ...
                        if (tg) {
                            ...
                            if (e.pageX < pPos.right && e.pageY > pPos.top) {
                                ...
                                this.chipsGroups.splice(this.chipsGroups.indexOf(tg), 1);
                            }
                            else {
                                if (this.dragMode == DragMode.NEW && tg.groupType == GroupType.CHIP) {
                                    ...
                                    if (count > 1) {
                                        tg.groupTag.removeChild(tg.groupTag.lastChild);
                                        for (var i = tg.groupTag.children.length - 1, j = 0; i >= 0; i--, j++) {
                                            if (i == 0) {
                                                tg.groupType = GroupType.CHIP;
                                                tg.groupTag.firstChild.setAttribute('x', '0');
                                                tg.groupTag.firstChild.setAttribute('y', '0');
                                                this.svgCanvas.SetGroupPositionValue(tg.groupTag, xy.x + (this.NSize + this.GAP) * j, xy.y);
                                                this.RemoveByGroupTag(tg.groupTag);
                                                this.chipsGroups.push(tg);
                                            }
                                            else {
                                                var newGroup = this.MakeNewGroupBase(xy.x + (this.NSize + this.GAP) * j, xy.y);
                                                newGroup.groupType = GroupType.CHIP;
                                                newGroup.groupTag.appendChild(tg.groupTag.children[i]);
                                                newGroup.groupTag.firstChild.setAttribute('x', '0');
                                                newGroup.groupTag.firstChild.setAttribute('y', '0');
            ...
            Chips.prototype.RemoveByGroupTag = function (grouptag) {
                var n = -1;
                for (var i = 0; i < this.chipsGroups.length; i++) {
                    if (this.chipsGroups[i].groupTag == grouptag) {
                        n = i;
                        break;
                    }
                }
                if (n >= 0)
                    this.chipsGroups.splice(n, 1);
            };
            Chips.prototype.MakeNewGroupBase = function (x, y) {
                var group = new ChipsGroup(this.svgCanvas);
                ...
                this.chipsGroups.push(group);
                return group;
            };
            Chips.prototype.MakeNewGroup = function (x, y, oldParent) {
                ...
                if (oldParent) {
                    ...
                    if (group.groupTag.children.length == 1) {
                        var c = group.groupTag.firstChild.COLORINDEXV;
                        group.SetColor(this.presetColors[c * 3], this.presetColors[c * 3 + 1], this.presetColors[c * 3 + 2]);
                    }
                }
                else {
            ...
            Chips.prototype.ShowHighlighted = function (group, flag) {
                if (group) {
                    if (flag) {
            ...
            Chips.prototype.SetSelectionBy = function (x, y, w, h) {
                var _a;
                for (var i = 0; i < this.chipsGroups.length; i++) {
                    var ar = this.chipsGroups[i].groupTag.getBoundingClientRect();
                    if (!(x > ar.right ||
                        x + w < ar.left ||
                        y > ar.bottom ||
                        y + h < ar.top)) {
                        (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.SelectItemAdd(this.chipsGroups[i]);
                    }
                }
            };
            Chips.prototype.Rotate = function (group, degree) {
            ...
            Chips.prototype.Delete = function (targetGroup) {
                if (targetGroup != null && this.chipsGroups.indexOf(targetGroup) >= 0) {
                    this.svgCanvas.RemoveGroup(targetGroup.groupTag);
                    this.chipsGroups.splice(this.chipsGroups.indexOf(targetGroup), 1);
                }
            };
            Chips.prototype.Duplicate = function (targetGroup) {
                var _a, _b;
                if (targetGroup != null) {
                    ...
                    this.svgCanvas.SetGroupPositionValue(groupB.groupTag, xy2.x + this.NSize / 2, xy2.y + 5);
            ...
            Chips.prototype.RestoreInitialState = function () {
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:400;');
                for (var i = 0; i < this.chipsGroups.length; i++) {
                    this.svgCanvas.RemoveGroup(this.chipsGroups[i].groupTag);
                }
                this.chipsGroups = [];
            };
            Chips.prototype.EnableInteraction = function () {
                this.svgCanvas.MoveToFront();
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:750;');
            };
            Chips.prototype.DisableInteraction = function () {
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:400;');
            };
            Chips.prototype.MakeText = function (txt) {
                var ttag = document.createElementNS('http://www.w3.org/2000/svg', 'text');
                ttag.setAttribute('x', (this.NSize / 2).toString());
                ttag.setAttribute('y', (this.NSize / 2 + 2).toString());
                ttag.textContent = txt;
                ttag.style.textAnchor = 'middle';
                ttag.style.dominantBaseline = "middle";
                ttag.style.font = 'bold 30px sans-serif';
                ttag.style.fill = 'rgba(255,255,255,0.5)';
                ttag.style.stroke = 'rgba(0,0,0,0.5)';
                ttag.style.strokeWidth = '1';
                ttag.style.pointerEvents = 'none';
                ttag.style.touchAction = 'none';
                ttag.style.userSelect = 'none';
                ttag.setAttribute('textLength', (this.NSize / 2 - 5).toString());
                ttag.setAttribute('lengthAdjust', 'spacingAndGlyphs');
                return ttag;
            };
            Chips.prototype.OnClickMerge = function () {
                var _a, _b;
                var chips = [];
                for (var i = 0; i < upred.ui.uiModule.selectedItems.length; i++) {
                    if (upred.ui.uiModule.selectedItems[i].groupType == GroupType.CHIP)
                        chips.push(upred.ui.uiModule.selectedItems[i]);
                }
                (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.UnselectAll();
                for (var j = 0; j < chips.length; j += 10) {
                    if (j + 9 >= chips.length)
                        break;
                    var x1 = 9999999, y1 = 9999999, x2 = 0, y2 = 0;
                    for (var i = j; i < j + 10; i++) {
                        var xy = this.svgCanvas.GetGroupPositionValue(chips[i].groupTag);
                        if (x1 > xy.x)
                            x1 = xy.x;
                        if (y1 > xy.y)
                            y1 = xy.y;
                    ...
                    group.groupType = GroupType.CHIPS10;
                    for (var i = j + 9, n = 9; i >= j; i--, n--) {
                        var xy = this.svgCanvas.GetGroupPositionValue(chips[i].groupTag);
                        chips[i].groupTag.firstChild.setAttribute('x', '0');
                        chips[i].groupTag.firstChild.setAttribute('y', (n * 5).toString());
                        chips[i].groupTag.firstChild.OLD_POS_X = xy.x;
                        chips[i].groupTag.firstChild.OLD_POS_Y = xy.y;
                        group.groupTag.appendChild(chips[i].groupTag.firstChild);
                        this.RemoveByGroupTag(chips[i].groupTag);
                        this.svgCanvas.RemoveGroup(chips[i].groupTag);
                    }
                    group.groupTag.appendChild(this.MakeText('10'));
                    this.svgCanvas.GetTag().appendChild(group.groupTag);
                    (_b = upred.ui.uiModule) === null || _b === void 0 ? void 0 : _b.SelectItemAdd(group);
                }
            };
            Chips.prototype.CanBeMerged = function () {
                var one = 0;
                for (var i = 0; i < upred.ui.uiModule.selectedItems.length; i++) {
                    if (upred.ui.uiModule.selectedItems[i].groupType == GroupType.CHIP)
                        one++;
            ...
            Chips.prototype.OnClickSplit = function () {
                var _a, _b;
                var chips10Arr = [];
                for (var i = 0; i < upred.ui.uiModule.selectedItems.length; i++) {
                    if (upred.ui.uiModule.selectedItems[i].groupType == GroupType.CHIPS10)
                        chips10Arr.push(upred.ui.uiModule.selectedItems[i]);
                }
                (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.UnselectAll();
                for (var i = 0; i < chips10Arr.length; i++) {
                    if (chips10Arr[i].groupType == GroupType.CHIPS10) {
                        var chips10 = chips10Arr[i];
                        var XY = this.svgCanvas.GetGroupPositionValue(chips10.groupTag);
                        for (var i_1 = 0; i_1 < 10; i_1++) {
                            var nx = XY.x + i_1 * this.NSize;
                            var ny = XY.y + i_1 * 5;
                            var tag = chips10.groupTag.children[chips10.groupTag.children.length - 2];
                            if (tag.OLD_POS_X)
                                nx = tag.OLD_POS_X;
                            if (tag.OLD_POS_Y)
                                ny = tag.OLD_POS_Y;
                            ...
                            group.groupTag.appendChild(tag);
                            this.svgCanvas.GetTag().appendChild(group.groupTag);
                            (_b = upred.ui.uiModule) === null || _b === void 0 ? void 0 : _b.SelectItemAdd(group);
                        }
                        this.RemoveByGroupTag(chips10.groupTag);
                        this.svgCanvas.RemoveGroup(chips10.groupTag);
                    }
                }
            };
            Chips.prototype.CanBeSplitted = function () {
                var ten = 0;
                for (var i = 0; i < upred.ui.uiModule.selectedItems.length; i++) {
                    if (upred.ui.uiModule.selectedItems[i].groupType == GroupType.CHIPS10)
                        ten++;
