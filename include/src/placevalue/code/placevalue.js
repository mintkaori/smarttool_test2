...
(function (upred) {
    ...
    (function (math) {
        var GridBoard = (function () {
            ...
            GridBoard.prototype.MakeGrids = function (width, height, cellSize, marginLeft) {
                var wc = Math.floor(width / cellSize);
                var hc = Math.floor(height / cellSize);
                this.marginLeft = marginLeft;
                this.bgTag = this.MakeTagStyled(this.groupTag, 'rect', { x: (marginLeft + cellSize / 2).toString(), y: (cellSize / 2).toString(), width: ((wc - 1) * cellSize).toString(), height: ((hc - 1) * cellSize).toString() }, { strokeWidth: '0', fill: 'transparent', lineCap: 'butt', touchAction: 'none', userSelect: 'none' });
                for (var i = 1; i < wc; i++) {
                    this.MakeTagStyled(this.groupTag, 'line', { x1: (marginLeft + i * cellSize).toString(), y1: cellSize.toString(), x2: (marginLeft + i * cellSize).toString(), y2: ((hc - 1) * cellSize).toString() }, { strokeWidth: '1', lineCap: 'butt', pointerEvents: 'none', touchAction: 'none', userSelect: 'none' });
                }
                for (var i = 1; i < hc; i++) {
                    this.MakeTagStyled(this.groupTag, 'line', { x1: (marginLeft + cellSize).toString(), y1: (i * cellSize).toString(), x2: (marginLeft + (wc - 1) * cellSize).toString(), y2: (i * cellSize).toString() }, { strokeWidth: '1', lineCap: 'butt', pointerEvents: 'none', touchAction: 'none', userSelect: 'none' });
                }
                this.px.xx = cellSize;
                this.px.yy = cellSize;
                this.px.sx = cellSize + marginLeft;
                this.px.sy = cellSize;
                this.px.ex = (wc - 2) * cellSize + marginLeft;
                this.px.ey = (hc - 2) * cellSize;
            };
            GridBoard.prototype.GetNearestPoint = function (mx, my) {
                var x, y;
                if (mx + this.px.xx < this.px.sx)
                    return null;
                if (my + this.px.yy < this.px.sy)
                    return null;
                if (mx - this.px.xx > this.px.ex)
                    return null;
                if (my - this.px.yy > this.px.ey)
                    return null;
                if (mx < this.px.sx)
                    x = this.px.sx;
                    ...
                    x = Math.round((mx - this.marginLeft) / this.px.xx) * this.px.xx + this.marginLeft;
                if (my < this.px.sy)
                    y = this.px.sy;
            ...
            GridBoard.prototype.Highlight = function (flag) {
                if (flag) {
                    this.bgTag.style.stroke = '#95A5A6';
                    this.bgTag.style.strokeWidth = '1';
                }
                else {
                    this.bgTag.style.stroke = 'transparent';
                    this.bgTag.style.strokeWidth = '0';
                }
            };
            return GridBoard;
        }());
    ...
    (function (math) {
        var PlaceCard = (function () {
            function PlaceCard(canvas) {
                ...
                this.groupTag.FINST = this;
            }
            PlaceCard.prototype.MakeCard = function (numText, unitW, unitH) {
                this.unitW = unitW;
                this.unitH = unitH;
                for (var i = 0; i < numText.length; i++) {
                    var rect = this.MakeTagStyled(this.groupTag, 'rect', { x: (i * unitW).toString(), y: '0', width: unitW.toString(), height: unitH.toString() }, { strokeWidth: this.lineThickness.toString(), touchAction: 'none', pointerEvents: 'visiblePainted', userSelect: 'none' });
                    var text = this.MakeTagStyled(this.groupTag, 'text', { x: (i * unitW + unitW / 2).toString(), y: '0', lengthAdjust: 'spacingAndGlyphs', textLength: (unitW * 0.7).toString(), 'text-anchor': 'middle' }, { fontSize: unitH + 'px', fontWeight: 'bold', fontFamily: 'sans-serif', filter: 'brightness(0.3)', dominantBaseline: 'hanging', strokeWidth: '0', touchAction: 'none', pointerEvents: 'visiblePainted', userSelect: 'none' });
                    text.textContent = numText[i];
                    this.rectTags.push(rect);
                    this.textTags.push(text);
                }
            };
            PlaceCard.prototype.MakeUpDownButtons = function () {
                this.upButton = this.MakeTagStyled(this.groupTag, 'image', { href: './asset/icon_up.png', x: (this.unitW / 4).toString(), y: (-this.unitW * 0.6).toString(), width: (this.unitW / 2).toString(), height: (this.unitW / 2).toString() }, { display: 'none', touchAction: 'none', userSelect: 'none', pointerEvents: 'visiblePainted' });
                this.downButton = this.MakeTagStyled(this.groupTag, 'image', { href: './asset/icon_down.png', x: (this.unitW / 4).toString(), y: (this.unitH + this.unitW * 0.1).toString(), width: (this.unitW / 2).toString(), height: (this.unitW / 2).toString() }, { display: 'none', touchAction: 'none', userSelect: 'none', pointerEvents: 'visiblePainted' });
                this.upButton.onpointerdown = this.OnUpButtonDown.bind(this);
                this.downButton.onpointerdown = this.OnDownButtonDown.bind(this);
            };
            PlaceCard.prototype.OnUpButtonDown = function (e) {
                var num = parseInt(this.textTags[0].textContent);
                if (++num > 9)
                    num = 0;
                this.textTags[0].textContent = num.toString();
                e.preventDefault();
                e.stopPropagation();
            };
            PlaceCard.prototype.OnDownButtonDown = function (e) {
                var num = parseInt(this.textTags[0].textContent);
                if (--num < 0)
                    num = 9;
                this.textTags[0].textContent = num.toString();
            ...
            PlaceCard.prototype.ShowUpDownButtons = function () {
                if (this.upButton && this.downButton) {
                    this.upButton.style.display = 'block';
                    this.downButton.style.display = 'block';
                }
            };
            PlaceCard.prototype.HideUpDownButtons = function () {
                if (this.upButton && this.downButton) {
                    this.upButton.style.display = 'none';
                    this.downButton.style.display = 'none';
                }
            };
            PlaceCard.prototype.GetDigitLength = function () {
                return this.textTags.length;
            };
            PlaceCard.prototype.Duplicate = function () {
                var pc = new PlaceCard(this.svgCanvas);
                pc.unitW = this.unitW;
                pc.unitH = this.unitH;
                for (var i = 0; i < this.groupTag.children.length; i++) {
                    var cp = this.groupTag.children[i].cloneNode(true);
                    pc.groupTag.appendChild(cp);
                    if (cp.tagName == 'RECT' || cp.tagName == 'rect') {
                        pc.rectTags.push(cp);
                    }
                    else if (cp.tagName == 'text' || cp.tagName == 'text') {
                        pc.textTags.push(cp);
                    }
                }
                if (this.upButton)
                    pc.MakeUpDownButtons();
                pc.SetColor(this.colorValue.r, this.colorValue.g, this.colorValue.b);
                return pc;
            };
            PlaceCard.prototype.SetColor = function (r, g, b) {
                this.colorValue.r = r;
                this.colorValue.g = g;
                this.colorValue.b = b;
            ...
            PlaceCard.prototype.MakeRGB = function (r, g, b, m) {
                r *= m;
                if (r < 0)
                    r = 0;
                if (r > 255)
            ...
            PlaceCard.prototype.MakeTagStyled = function (parent, tagname, attr, style) {
                var node = document.createElementNS('http://www.w3.org/2000/svg', tagname);
                for (var key in attr) {
                    if (attr.hasOwnProperty(key)) {
                        node.setAttribute(key, attr[key]);
            ...
            PlaceCard.prototype.Highlight = function (flag) {
                if (flag) {
...
(function (upred) {
    ...
    (function (math) {
        ...
        (function (DragMode) {
            ...
            DragMode[DragMode["NEW"] = 1] = "NEW";
            DragMode[DragMode["MOVE"] = 2] = "MOVE";
        })(DragMode || (DragMode = {}));
        var PlaceValue = (function () {
            function PlaceValue() {
            ...
            PlaceValue.prototype.HexToRGB = function (hex) {
                if (hex[0] == '#') {
            ...
            PlaceValue.prototype.ChangeColor = function (item, colrstr) {
                var color = this.HexToRGB(colrstr);
                if (item.SetColor) {
                    item.SetColor(color.r, color.g, color.b);
                }
            };
            PlaceValue.prototype.OnDragStartNew = function (e) {
                var _a;
                if (!e.currentTarget)
                    return;
                if (!e.currentTarget.parentNode)
                    return;
                if (!e.currentTarget.parentNode.FINST)
                    return;
            ...
            PlaceValue.prototype.CompareDigit = function (a, b) {
                if (a.dn > b.dn)
                    return -1;
                else if (a.dn == b.dn)
                    return 0;
            ...
            PlaceValue.prototype.Sort = function () {
                var ch = [];
                for (var i = 0; i < this.allObjs.length; i++) {
                    if (this.allObjs[i].type != 'card')
                        continue;
                    var xy = this.svgCanvas.GetGroupPositionValue(this.allObjs[i].groupTag);
                    ch.push({ pos: xy, item: this.allObjs[i], dn: this.allObjs[i].GetDigitLength() });
                    this.svgCanvas.RemoveGroup(this.allObjs[i].groupTag);
                }
                this.allObjs = [];
                ch.sort(this.CompareDigit.bind(this));
                for (var i = 0; i < ch.length; i++) {
                    this.svgCanvas.AddGroupExists(ch[i].item.groupTag, ch[i].pos.x, ch[i].pos.y);
                    this.allObjs.push(ch[i].item);
                }
            };
            PlaceValue.prototype.OnDragStartMove = function (e) {
                var _a, _b;
                var usetag = e.target;
                if (!usetag)
                    return;
                if (this.dragMode == DragMode.NONE) {
                    ...
                    for (var i = 0; i < this.allObjs.length; i++) {
                        if (this.allObjs[i].type == 'card' && this.allObjs[i].groupTag == usetag.parentNode) {
            ...
            PlaceValue.prototype.OnDragging = function (e) {
                if (!this.curDragging)
                    return;
                var dx = e.pageX - this.xyBefore.x;
                var dy = e.pageY - this.xyBefore.y;
                if (this.curDragging.type != 'card')
                    return;
                var near = this.curBoard.GetNearestPoint(this.xyOrg.x + dx, this.xyOrg.y + dy);
                if (near) {
                    this.curDragging.groupTag.style.opacity = '1';
                    this.svgCanvas.SetGroupPositionValue(this.curDragging.groupTag, near.x, near.y);
                }
                else {
            ...
            PlaceValue.prototype.OnDragEnd = function (e) {
                var _a, _b;
                if (!this.curDragging)
                    return;
                ...
                if (near) {
                    ...
                    if (dx * dx + dy * dy < 32) {
                        (_b = upred.ui.uiModule) === null || _b === void 0 ? void 0 : _b.SelectItemOnlyOne(this.curDragging);
                        this.curDragging.ShowUpDownButtons();
                    }
                    else {
                        this.curDragging.HideUpDownButtons();
                    }
                }
                else {
            ...
            PlaceValue.prototype.AssignHandlerAndRegister = function (tline, x, y) {
            ...
            PlaceValue.prototype.ShowHighlighted = function (group, flag) {
                if (group) {
                    if (group.Highlight)
                        group.Highlight(flag);
                }
            };
            PlaceValue.prototype.SetSelectionBy = function (x, y, w, h) {
                var _a;
                for (var i = 0; i < this.allObjs.length; i++) {
            ...
            PlaceValue.prototype.Rotate = function (group, degree) {
            };
            PlaceValue.prototype.Delete = function (targetGroup) {
            ...
            PlaceValue.prototype.Duplicate = function (targetGroup) {
                var copy = targetGroup.Duplicate();
                var xy = this.svgCanvas.GetGroupPositionValue(targetGroup.groupTag);
                this.AssignHandlerAndRegister(copy, xy.x, xy.y + this.UNIT_SIZE);
            };
            PlaceValue.prototype.RestoreInitialState = function () {
            ...
            PlaceValue.prototype.EnableInteraction = function () {
                this.svgCanvas.MoveToFront();
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:750;');
            };
            PlaceValue.prototype.DisableInteraction = function () {
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:400;');
            };
            PlaceValue.prototype.InitBoard = function () {
                ...
                var wh = this.curBoard.MakeGrids(RW, RH, this.UNIT_SIZE, SX);
            ...
            PlaceValue.prototype.DeleteChecker = function () {
            ...
            PlaceValue.prototype.DuplicationChecker = function () {
                if (upred.ui.uiModule.selectedItems.length <= 1) {
                    if (upred.ui.uiModule.selectedItems[0].type
                        && upred.ui.uiModule.selectedItems[0].type != 'board')
                        return true;
            ...
            PlaceValue.prototype.InitPalette = function () {
                ...
                for (var i = 0; i < 4; i++) {
                    var cardBase = new upred.ui.SVGUnit();
                    var tng = new math.PlaceCard(this.svgCanvas);
                    tng.MakeCard(Math.pow(10, i).toString(), this.UNIT_SIZE, this.UNIT_SIZE);
                    tng.SetColor(colors[i * 3], colors[i * 3 + 1], colors[i * 3 + 2]);
                    for (var j = 0; j < tng.rectTags.length; j++) {
                        tng.rectTags[j].addEventListener('pointerdown', this.OnDragStartNew.bind(this));
                        tng.textTags[j].addEventListener('pointerdown', this.OnDragStartNew.bind(this));
                    }
                    cardBase.GetTag().appendChild(tng.groupTag);
                    cardBase.GetTag().style.display = 'inline-block';
                    cardBase.GetTag().style.margin = '8px 8px';
                    cardBase.SetSize((this.UNIT_SIZE * (i + 1)), this.UNIT_SIZE);
                    this.paletteTag.appendChild(cardBase.GetTag());
                }
                ...
                this.svgCanvas.dragStarter.text = this.OnDragStartMove.bind(this);
            };
            PlaceValue.prototype.Init = function () {
                var _a;
                this.svgCanvas = new upred.ui.SVGCanvas();
        ...
        math.PlaceValue = PlaceValue;
    })(math = upred.math || (upred.math = {}));
})(upred || (upred = {}));
//# sourceMappingURL=placevalue.js.map