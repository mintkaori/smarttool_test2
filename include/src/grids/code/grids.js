...
(function (upred) {
    ...
    (function (math) {
        var GridBoard = (function () {
            function GridBoard(canvas) {
                this.type = "board";
                this.px = { sx: 0, sy: 0, ex: 0, ey: 0, xx: 0, yy: 0 };
                this.groupTag = canvas.AddGroup();
                upred.ui.HTML.setStyle(this.groupTag, {
                    position: 'absolute',
                });
            }
            GridBoard.prototype.SetColor = function (r, g, b) {
                var bright = this.MakeRGB(r, g, b, 1.5);
                var dark = this.MakeRGB(r, g, b, 1);
                this.groupTag.style.fill = dark;
                this.groupTag.style.stroke = bright;
            };
            GridBoard.prototype.MakeRGB = function (r, g, b, m) {
                r *= m;
                if (r < 0)
                    r = 0;
                if (r > 255)
            ...
            GridBoard.prototype.MakeTagStyled = function (parent, tagname, attr, style) {
            ...
            GridBoard.prototype.MakeGrids = function (width, height, cellSize) {
                var wc = Math.floor(width / cellSize);
                var hc = Math.floor(height / cellSize);
                this.MakeTagStyled(this.groupTag, 'rect', { x: (cellSize / 2).toString(), y: (cellSize / 2).toString(), width: ((wc - 1) * cellSize).toString(), height: ((hc - 1) * cellSize).toString() }, { strokeWidth: '0', fill: 'transparent', lineCap: 'butt', touchAction: 'none', userSelect: 'none' });
                for (var i = 1; i < wc; i++) {
                    this.MakeTagStyled(this.groupTag, 'line', { x1: (i * cellSize).toString(), y1: cellSize.toString(), x2: (i * cellSize).toString(), y2: ((hc - 1) * cellSize).toString() }, { strokeWidth: '1', lineCap: 'butt', pointerEvents: 'none', touchAction: 'none', userSelect: 'none' });
                }
                for (var i = 1; i < hc; i++) {
                    this.MakeTagStyled(this.groupTag, 'line', { x1: cellSize.toString(), y1: (i * cellSize).toString(), x2: ((wc - 1) * cellSize).toString(), y2: (i * cellSize).toString() }, { strokeWidth: '1', lineCap: 'butt', pointerEvents: 'none', touchAction: 'none', userSelect: 'none' });
                }
                for (var y = 1; y < hc; y++) {
                    for (var x = 1; x < wc; x++) {
                        var cx = (x * cellSize);
                        var cy = (y * cellSize);
                        this.MakeTagStyled(this.groupTag, 'circle', { cx: cx.toString(), cy: cy.toString(), r: GridBoard.POINT_RADIUS.toString() }, { stroke: 'none', strokeWidth: '0', pointerEvents: 'none', touchAction: 'none', userSelect: 'none' });
                    }
                }
                this.px.xx = cellSize;
                this.px.yy = cellSize;
                this.px.sx = cellSize;
                this.px.sy = cellSize;
                this.px.ex = (wc - 1) * cellSize;
                this.px.ey = (hc - 1) * cellSize;
            };
            GridBoard.prototype.GetNearestPoint = function (mx, my) {
                var x, y;
                if (mx < this.px.sx)
                    x = this.px.sx;
                else if (mx > this.px.ex)
                    x = this.px.ex;
                else
                    x = Math.round(mx / this.px.xx) * this.px.xx;
                if (my < this.px.sy)
                    y = this.px.sy;
                else if (my > this.px.ey)
                    y = this.px.ey;
                else
                    y = Math.round(my / this.px.yy) * this.px.yy;
                return { x: x, y: y };
            };
            GridBoard.POINT_RADIUS = 2;
            return GridBoard;
        }());
        math.GridBoard = GridBoard;
    ...
    (function (math) {
        var GridLine = (function () {
            function GridLine(canvas) {
                ...
                this.lineTag = this.MakeTagStyled(this.groupTag, 'line', {
                    x1: '0', y1: '0', x2: '1', y2: '1'
                }, { strokeWidth: this.lineThickness.toString(), fill: 'none', opacity: '0' });
                this.point1Tag = this.MakeTagStyled(this.groupTag, 'circle', {
                    cx: '0', cy: '0', r: '9'
                }, { strokeWidth: '0', opacity: '1' });
                this.point2Tag = this.MakeTagStyled(this.groupTag, 'circle', {
                    cx: '0', cy: '0', r: '9'
                }, { strokeWidth: '0', opacity: '1' });
            }
            GridLine.prototype.SetColor = function (r, g, b) {
                var c = this.MakeRGB(r, g, b, 1);
                this.groupTag.style.stroke = c;
                this.groupTag.style.fill = c;
            };
            GridLine.prototype.MakeRGB = function (r, g, b, m) {
                r *= m;
                if (r < 0)
                    r = 0;
                if (r > 255)
            ...
            GridLine.prototype.MakeTagStyled = function (parent, tagname, attr, style) {
                var node = document.createElementNS('http://www.w3.org/2000/svg', tagname);
                for (var key in attr) {
                    if (attr.hasOwnProperty(key)) {
                        node.setAttribute(key, attr[key]);
            ...
            GridLine.prototype.Highlight = function (flag) {
                if (flag) {
...
(function (upred) {
    var math;
    (function (math) {
        var GridPolygon = (function () {
            function GridPolygon(canvas) {
                this.lineThickness = 5;
                this.type = "polygon";
                this.points = [];
                this.groupTag = canvas.AddGroup();
                ...
                this.fillPathTag = this.MakeTagStyled(this.groupTag, 'path', {}, {
                    strokeWidth: '0',
                    opacity: '0',
                    lineCap: 'round',
                    lineJoin: 'bevel'
                ...
                this.pathTag = this.MakeTagStyled(this.groupTag, 'path', {}, {
                    strokeWidth: this.lineThickness.toString(),
                    fill: 'none',
                    opacity: '0',
                    lineCap: 'round',
                    lineJoin: 'bevel'
                });
                this.pointTags = this.MakeTagStyled(this.groupTag, 'g', {}, { position: 'absolute' });
                this.AddPoint(0, 0);
            }
            GridPolygon.prototype.AddPoint = function (x, y) {
                this.points.push({ x: x, y: y });
                var circle = this.MakeTagStyled(this.pointTags, 'circle', {
                    cx: x.toString(), cy: y.toString(), r: '9'
                }, { strokeWidth: '0', opacity: '1' });
                this.UpdateShape();
                return circle;
            };
            GridPolygon.prototype.MovePoint = function (circle, nx, ny) {
                var idx = -1;
                for (var i = 0; i < this.pointTags.children.length; i++) {
                    if (this.pointTags.children[i] == circle) {
                        idx = i;
                        break;
                    }
                }
                if (idx < 0)
                    return;
                circle.setAttribute('cx', nx.toString());
                circle.setAttribute('cy', ny.toString());
                this.points[idx].x = nx;
                this.points[idx].y = ny;
                this.UpdateShape();
            };
            GridPolygon.prototype.UpdateShape = function () {
                var dstr = '';
                for (var i = 0; i < this.points.length; i++) {
                    if (i == 0)
                        ...
                        dstr += ' L ';
                    dstr += this.points[i].x;
                    dstr += ' ';
                    dstr += this.points[i].y;
                }
                ...
                if (this.points.length > 1) {
                    this.pathTag.setAttribute('d', dstr);
                    this.pathTag.style.opacity = '1';
                    this.fillPathTag.setAttribute('d', dstr);
                    if (this.points[this.points.length - 1].x == this.points[0].x
                        && this.points[this.points.length - 1].y == this.points[0].y) {
                        this.fillPathTag.style.opacity = '0.35';
                    }
                    else {
                        this.fillPathTag.style.opacity = '0.1';
                    }
                }
                else {
                    this.pathTag.style.opacity = '0';
                    this.fillPathTag.style.opacity = '0';
                }
            };
            GridPolygon.prototype.SetColor = function (r, g, b) {
                var c = this.MakeRGB(r, g, b, 1);
                this.groupTag.style.stroke = c;
                this.groupTag.style.fill = c;
            };
            GridPolygon.prototype.MakeRGB = function (r, g, b, m) {
                r *= m;
                if (r < 0)
                    r = 0;
                if (r > 255)
            ...
            GridPolygon.prototype.MakeTagStyled = function (parent, tagname, attr, style) {
                var node = document.createElementNS('http://www.w3.org/2000/svg', tagname);
                for (var key in attr) {
                    if (attr.hasOwnProperty(key)) {
                        node.setAttribute(key, attr[key]);
            ...
            GridPolygon.prototype.Highlight = function (flag) {
                if (flag) {
                    this.groupTag.setAttribute('filter', 'url(#filterOutline)');
                }
                else {
...
(function (upred) {
    ...
    (function (math) {
        ...
        (function (DragMode) {
            ...
            DragMode[DragMode["NEW_POINT_MONITOR"] = 1] = "NEW_POINT_MONITOR";
            DragMode[DragMode["NEW_POLY_MONITOR"] = 2] = "NEW_POLY_MONITOR";
            DragMode[DragMode["NEW_POINT2_DRAGGING"] = 3] = "NEW_POINT2_DRAGGING";
            DragMode[DragMode["NEW_POLY_NEXT_POINT"] = 4] = "NEW_POLY_NEXT_POINT";
            DragMode[DragMode["MOVE_POINT"] = 5] = "MOVE_POINT";
            DragMode[DragMode["MOVE_LINE"] = 6] = "MOVE_LINE";
        })(DragMode || (DragMode = {}));
        var Grids = (function () {
            function Grids() {
            ...
            Grids.prototype.HexToRGB = function (hex) {
                if (hex[0] == '#') {
            ...
            Grids.prototype.ChangeColor = function (item, colrstr) {
                var color = this.HexToRGB(colrstr);
                if (item.SetColor) {
                    item.SetColor(color.r, color.g, color.b);
                }
            };
            Grids.prototype.OnDragStart = function (e) {
                var _a, _b;
                var usetag = e.target;
                if (!usetag)
                    return;
                if (this.dragMode == DragMode.NONE) {
                    ...
                    for (var i = 0; i < this.allObjs.length; i++) {
                        if ((this.allObjs[i].type == 'line' || this.allObjs[i].type == 'polygon')
                            && (this.allObjs[i].groupTag == usetag.parentNode)
                            || (this.allObjs[i].pointTags == usetag.parentNode)) {
                    ...
                    if (!this.curDragging) {
                        (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.SelectItemOnlyOne(this.curBoard);
                        e.preventDefault();
                        return;
                    }
                    else {
                        if (this.curDraggingTag.tagName == 'line' || this.curDraggingTag.tagName == 'LINE'
                            || this.curDraggingTag.tagName == 'path' || this.curDraggingTag.tagName == 'PATH') {
                            this.dragMode = DragMode.MOVE_LINE;
                            var gxy = this.svgCanvas.GetGroupPositionValue(this.curDragging.groupTag);
                            this.xyOrg.x = gxy.x;
                            this.xyOrg.y = gxy.y;
                        }
                        else if (this.curDraggingTag.tagName == 'circle' || this.curDraggingTag.tagName == 'CIRCLE') {
                            this.dragMode = DragMode.MOVE_POINT;
                    ...
                    var nxy = this.curBoard.GetNearestPoint(e.pageX, e.pageY);
                    this.xyNow.x = this.xyBefore.x = nxy.x;
                    this.xyNow.y = this.xyBefore.y = nxy.y;
                    e.preventDefault();
                }
                else if (this.dragMode == DragMode.NEW_POINT_MONITOR || this.dragMode == DragMode.NEW_POLY_MONITOR) {
                    if (this.dragMode == DragMode.NEW_POLY_MONITOR)
                        this.curDragging = new math.GridPolygon(this.svgCanvas);
                    else
                        this.curDragging = new math.GridLine(this.svgCanvas);
                    this.curDragging.SetColor(231, 76, 60);
                    var nxy = this.curBoard.GetNearestPoint(e.pageX, e.pageY);
                    this.xyOrg.x = this.xyNow.x = this.xyBefore.x = nxy.x;
                    this.xyOrg.y = this.xyNow.y = this.xyBefore.y = nxy.y;
                    this.AssignHandlerAndRegister(this.curDragging, this.xyNow.x, this.xyNow.y);
                    e.preventDefault();
                }
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:900;');
                return true;
            };
            Grids.prototype.OnDragging = function (e) {
                if (!this.curDragging)
                    return;
                var near = this.curBoard.GetNearestPoint(e.pageX, e.pageY);
                this.xyNow.x = near.x;
                this.xyNow.y = near.y;
                var dx = this.xyNow.x - this.xyBefore.x;
                var dy = this.xyNow.y - this.xyBefore.y;
                if (this.curDragging.type == 'line' || this.curDragging.type == 'polygon') {
                    switch (this.dragMode) {
                        case DragMode.NEW_POLY_MONITOR:
                        case DragMode.NEW_POINT_MONITOR:
                            {
                                this.svgCanvas.SetGroupPositionValue(this.curDragging.groupTag, this.xyOrg.x + dx, this.xyOrg.y + dy);
                            }
                            break;
                        case DragMode.NEW_POINT2_DRAGGING:
                            {
                                this.curDragging.point2Tag.setAttribute('cx', (this.xyOrg.x + dx).toString());
                                this.curDragging.point2Tag.setAttribute('cy', (this.xyOrg.y + dy).toString());
                                if (dx != 0 || dy != 0) {
                                    this.curDragging.lineTag.setAttribute('x2', (this.xyOrg.x + dx).toString());
                                    this.curDragging.lineTag.setAttribute('y2', (this.xyOrg.y + dy).toString());
                                    this.curDragging.lineTag.style.opacity = '1';
                                }
                                else {
                                    this.curDragging.lineTag.style.opacity = '0';
                                }
                            }
                            break;
                        case DragMode.NEW_POLY_NEXT_POINT:
                            {
                                this.curDragging.MovePoint(this.curDraggingTag, this.xyOrg.x + dx, this.xyOrg.y + dy);
                            }
                            break;
                        case DragMode.MOVE_LINE:
                            {
                                this.svgCanvas.SetGroupPositionValue(this.curDragging.groupTag, this.xyOrg.x + dx, this.xyOrg.y + dy);
                            }
                            break;
                        case DragMode.MOVE_POINT:
                            {
                                if (this.curDragging.type == 'line') {
                                    this.curDraggingTag.setAttribute('cx', (this.xyOrg.x + dx).toString());
                                    this.curDraggingTag.setAttribute('cy', (this.xyOrg.y + dy).toString());
                                    var x1 = parseFloat(this.curDragging.point1Tag.getAttribute('cx'));
                                    var y1 = parseFloat(this.curDragging.point1Tag.getAttribute('cy'));
                                    var x2 = parseFloat(this.curDragging.point2Tag.getAttribute('cx'));
                                    var y2 = parseFloat(this.curDragging.point2Tag.getAttribute('cy'));
                                    if (x1 == x2 && y1 == y2) {
                                        this.curDragging.lineTag.style.opacity = '0';
                                    }
                                    else {
                                        if (this.curDragging.point1Tag == this.curDraggingTag) {
                                            this.curDragging.lineTag.setAttribute('x1', (this.xyOrg.x + dx).toString());
                                            this.curDragging.lineTag.setAttribute('y1', (this.xyOrg.y + dy).toString());
                                            this.curDragging.lineTag.setAttribute('x2', (x2).toString());
                                            this.curDragging.lineTag.setAttribute('y2', (y2).toString());
                                        }
                                        else {
                                            this.curDragging.lineTag.setAttribute('x2', (this.xyOrg.x + dx).toString());
                                            this.curDragging.lineTag.setAttribute('y2', (this.xyOrg.y + dy).toString());
                                            this.curDragging.lineTag.setAttribute('x1', (x1).toString());
                                            this.curDragging.lineTag.setAttribute('y1', (y1).toString());
                                        }
                                    }
                                }
                                else if (this.curDragging.type == 'polygon') {
                                    this.curDragging.MovePoint(this.curDraggingTag, this.xyOrg.x + dx, this.xyOrg.y + dy);
                                }
                            }
                            break;
            ...
            Grids.prototype.OnDragEnd = function (e) {
                ...
                switch (this.dragMode) {
                    case DragMode.NEW_POINT_MONITOR:
                        {
                            this.dragMode = DragMode.NEW_POINT2_DRAGGING;
                            this.xyOrg.x = 0;
                            this.xyOrg.y = 0;
                            var near = this.curBoard.GetNearestPoint(e.pageX, e.pageY);
                            this.xyBefore.x = this.xyNow.x = near.x;
                            this.xyBefore.y = this.xyNow.y = near.y;
                        }
                        break;
                    case DragMode.NEW_POLY_MONITOR:
                        {
                            this.dragMode = DragMode.NEW_POLY_NEXT_POINT;
                            this.xyOrg.x = 0;
                            this.xyOrg.y = 0;
                            var near = this.curBoard.GetNearestPoint(e.pageX, e.pageY);
                            this.xyBefore.x = this.xyNow.x = near.x;
                            this.xyBefore.y = this.xyNow.y = near.y;
                            this.curDraggingTag = this.curDragging.AddPoint(0, 0);
                        }
                        break;
                    case DragMode.NEW_POINT2_DRAGGING:
                        {
                            this.curDragging.Highlight(true);
                            (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.UnselectAll();
                            (_b = upred.ui.uiModule) === null || _b === void 0 ? void 0 : _b.SelectItemOnlyOne(this.curDragging);
                            this.curDragging = undefined;
                            this.dragMode = DragMode.NONE;
                            (_c = upred.ui.uiModule) === null || _c === void 0 ? void 0 : _c.SelectButton(null);
                        }
                        break;
                    case DragMode.NEW_POLY_NEXT_POINT:
                        {
                            if (this.xyOrg.x + dx == 0 && this.xyOrg.y + dy == 0) {
                                this.curDragging.Highlight(true);
                                (_d = upred.ui.uiModule) === null || _d === void 0 ? void 0 : _d.UnselectAll();
                                (_e = upred.ui.uiModule) === null || _e === void 0 ? void 0 : _e.SelectItemOnlyOne(this.curDragging);
                                this.curDragging = undefined;
                                this.dragMode = DragMode.NONE;
                                (_f = upred.ui.uiModule) === null || _f === void 0 ? void 0 : _f.SelectButton(null);
                            }
                            else {
                                var near = this.curBoard.GetNearestPoint(e.pageX, e.pageY);
                                var pxy = this.svgCanvas.GetGroupPositionValue(this.curDragging.groupTag);
                                this.xyOrg.x = near.x - pxy.x;
                                this.xyOrg.y = near.y - pxy.y;
                                this.xyBefore.x = this.xyNow.x = near.x;
                                this.xyBefore.y = this.xyNow.y = near.y;
                                this.curDraggingTag = this.curDragging.AddPoint(this.xyOrg.x, this.xyOrg.y);
                            }
                        }
                        break;
                    case DragMode.MOVE_LINE:
                        {
                            this.dragMode = DragMode.NONE;
                            (_g = upred.ui.uiModule) === null || _g === void 0 ? void 0 : _g.UnselectAll();
                            if (dx * dx + dy * dy < 32) {
                                (_h = upred.ui.uiModule) === null || _h === void 0 ? void 0 : _h.SelectItemOnlyOne(this.curDragging);
                            }
                            this.curDragging = undefined;
                        }
                        break;
                    case DragMode.MOVE_POINT:
                        {
                            this.dragMode = DragMode.NONE;
                            (_j = upred.ui.uiModule) === null || _j === void 0 ? void 0 : _j.UnselectAll();
                            if (dx * dx + dy * dy < 32) {
                                (_k = upred.ui.uiModule) === null || _k === void 0 ? void 0 : _k.SelectItemOnlyOne(this.curDragging);
                            }
                            this.curDragging = undefined;
                        }
                        break;
            ...
            Grids.prototype.AssignHandlerAndRegister = function (tline, x, y) {
            ...
            Grids.prototype.ShowHighlighted = function (group, flag) {
                if (group && (group.type == 'line' || group.type == 'polygon')) {
                    if (flag) {
            ...
            Grids.prototype.SetSelectionBy = function (x, y, w, h) {
                var _a;
                for (var i = 0; i < this.allObjs.length; i++) {
            ...
            Grids.prototype.Rotate = function (group, degree) {
            };
            Grids.prototype.Delete = function (targetGroup) {
            ...
            Grids.prototype.Duplicate = function (targetGroup) {
            };
            Grids.prototype.RestoreInitialState = function () {
            ...
            Grids.prototype.EnableInteraction = function () {
                this.svgCanvas.MoveToFront();
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:750;');
            };
            Grids.prototype.DisableInteraction = function () {
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:400;');
            };
            Grids.prototype.OnClickLine = function () {
                var _a, _b;
                if (this.dragMode == DragMode.NEW_POINT_MONITOR) {
                    this.dragMode = DragMode.NONE;
                    (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.SelectButton(null);
                }
                else {
                    this.dragMode = DragMode.NEW_POINT_MONITOR;
                    (_b = upred.ui.uiModule) === null || _b === void 0 ? void 0 : _b.SelectButton(upred.ui.uiModule.uiButtons.drawLine);
                }
                this.EnableInteraction();
            };
            Grids.prototype.OnClickPoly = function () {
                var _a, _b;
                if (this.dragMode == DragMode.NEW_POLY_MONITOR) {
                    this.dragMode = DragMode.NONE;
                    (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.SelectButton(null);
                    ...
                    this.dragMode = DragMode.NEW_POLY_MONITOR;
                    (_b = upred.ui.uiModule) === null || _b === void 0 ? void 0 : _b.SelectButton(upred.ui.uiModule.uiButtons.drawPoly);
                }
                this.EnableInteraction();
            };
            Grids.prototype.InitBoards = function () {
                ...
                this.curBoard = new math.GridBoard(this.svgCanvas);
                var wh = this.curBoard.MakeGrids(RW, RH, 40);
                this.curBoard.SetColor(131, 145, 146);
                this.AssignHandlerAndRegister(this.curBoard, 0, 0);
                this.svgCanvas.GetTag().appendChild(this.curBoard.groupTag);
            };
            Grids.prototype.DeleteChecker = function () {
            ...
            Grids.prototype.Init = function () {
                ...
                (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.AddCustomPresetButton('drawPoly', {
                    img: './asset/icon_polygon.png',
                    txt: '다각형',
                    handler: this.OnClickPoly.bind(this)
                });
                (_b = upred.ui.uiModule) === null || _b === void 0 ? void 0 : _b.AddCustomPresetButton('drawLine', {
                    img: './asset/icon_line.png',
                    txt: '선분',
                    handler: this.OnClickLine.bind(this)
                });
                (_c = upred.ui.uiModule) === null || _c === void 0 ? void 0 : _c.DisableRotation();
                (_d = upred.ui.uiModule) === null || _d === void 0 ? void 0 : _d.DisableDuplication();
                this.InitBoards();
                this.svgCanvas.dragStarter.line = this.OnDragStart.bind(this);
                this.svgCanvas.dragStarter.circle = this.OnDragStart.bind(this);
                this.svgCanvas.dragStarter.rect = this.OnDragStart.bind(this);
                this.svgCanvas.dragStarter.path = this.OnDragStart.bind(this);
