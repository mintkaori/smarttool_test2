...
(function (upred) {
    ...
    (function (math) {
        ...
        var TangramFigure = (function () {
            function TangramFigure(canvas) {
                ...
                this.pathTag = this.MakeTagStyled(this.groupTag, 'path', {}, { strokeWidth: '2', lineJoin: 'miter',
                    pointerEvents: 'visiblePainted', touchAction: 'none', userSelect: 'none' });
                this.pathTag.FINST = this;
            }
            TangramFigure.prototype.SetColor = function (r, g, b) {
                var styletext = 'fill:' +
            ...
            TangramFigure.prototype.MakeTagStyled = function (parent, tagname, attr, style) {
            ...
            TangramFigure.prototype.MakeRGB = function (r, g, b, m) {
                r *= m;
                if (r < 0)
                    r = 0;
                if (r > 255)
            ...
            TangramFigure.prototype.MakeFigure = function (xy, baseSize, targetSize) {
                var dstr = 'M ';
                for (var i = 0; i < xy.length; i += 2) {
                    if (i > 0)
                        dstr += 'L ';
                    var x = (xy[i] / baseSize * targetSize);
                    var y = (xy[i + 1] / baseSize * targetSize);
                    dstr += x + ' ' + y + ' ';
                    this.shapeXY.push(xy[i]);
                    this.shapeXY.push(xy[i + 1]);
                    if (this.allWidth < x)
                        this.allWidth = x;
                    if (this.allHeight < y)
                        this.allHeight = y;
                    this.screenXY.push(new Point(x, y));
                }
                dstr += 'Z';
                this.pathTag.setAttribute('d', dstr);
            };
            TangramFigure.prototype.GetCenter = function () {
                var area = 0;
                var cx = 0, cy = 0;
                for (var i = 0; i < this.screenXY.length; i++) {
                    var nextI = (i + 1) % this.screenXY.length;
                    var x1 = this.screenXY[i].x;
                    var x2 = this.screenXY[nextI].x;
                    var y1 = this.screenXY[i].y;
                    var y2 = this.screenXY[nextI].y;
                    area += x1 * y2;
                    area -= y1 * x2;
            ...
            TangramFigure.prototype.GetMax = function () {
                var max = new Point(-99999, -99999);
                for (var i = 0; i < this.screenXY.length; i++) {
                    if (max.x < Math.abs(this.screenXY[i].x))
                        max.x = Math.abs(this.screenXY[i].x);
                    if (max.y < Math.abs(this.screenXY[i].y))
                        max.y = Math.abs(this.screenXY[i].y);
                }
                return max;
            };
            TangramFigure.prototype.MakeRotater = function (targetSize) {
            ...
            TangramFigure.prototype.UpdateRotater = function (x, y) {
            ...
            TangramFigure.prototype.ShowRotater = function (flag) {
                if (this.rotaterCircle && this.rotaterLine) {
                    if (!flag) {
            ...
            TangramFigure.prototype.RotatePoint = function (p, pv, rad) {
            ...
            TangramFigure.prototype.Rotate = function (degree) {
                var dstr = 'M ';
                var rad = degree * Math.PI / 180;
                var center = this.GetCenter();
                for (var i = 0; i < this.screenXY.length; i++) {
                    var r = this.RotatePoint(this.screenXY[i], center, rad);
                    this.screenXY[i].x = r.x;
                    this.screenXY[i].y = r.y;
                    if (i > 0)
                        dstr += 'L ';
                    dstr += this.screenXY[i].x + ' ' + this.screenXY[i].y + ' ';
                }
                dstr += 'Z';
                this.pathTag.setAttribute('d', dstr);
            };
            TangramFigure.prototype.Flip = function () {
                var min = 99999;
                var max = -99999;
                for (var i = 0; i < this.screenXY.length; i++) {
                    if (min > this.screenXY[i].x)
                        min = this.screenXY[i].x;
                    if (max < this.screenXY[i].x)
                        max = this.screenXY[i].x;
                }
                var dstr = 'M ';
                var center = (max + min) / 2;
                for (var i = 0; i < this.screenXY.length; i++) {
                    this.screenXY[i].x = center - (this.screenXY[i].x - center);
                    if (i > 0)
                        dstr += 'L ';
                    dstr += this.screenXY[i].x + ' ' + this.screenXY[i].y + ' ';
                }
            ...
            TangramFigure.prototype.Locate = function (x, y) {
                this.groupTag.setAttribute('transform', 'translate(' + x + ',' + y + ')');
                this.groupPos.x = x;
                this.groupPos.y = y;
            };
            TangramFigure.prototype.RepositionIn = function () {
                this.groupPos.x = (this.orgPosition.x / 20 * this.orgSize);
                this.groupPos.y = (this.orgPosition.y / 20 * this.orgSize);
                this.groupTag.setAttribute('transform', 'translate(' + this.groupPos.x + ',' + this.groupPos.y + ')');
            };
            TangramFigure.prototype.Duplicate = function (nsize, newflag) {
                if (newflag === void 0) { newflag = false; }
                var copy = new TangramFigure(this.svgCanvas);
                copy.MakeFigure(this.shapeXY, 20, nsize);
                copy.orgPosition.x = this.orgPosition.x;
                copy.orgPosition.y = this.orgPosition.y;
                copy.SetColor(this.color.r, this.color.g, this.color.b);
                copy.isAlwaysUnder = this.isAlwaysUnder;
                if (copy.isAlwaysUnder && this.svgCanvas.GetTag().children.length > 1) {
                    this.svgCanvas.GetTag().insertBefore(copy.groupTag, this.svgCanvas.GetTag().firstChild);
                }
                if (this.rotaterCircle) {
                    copy.MakeRotater(this.rotaterHandleRadius * 10);
                }
                if (newflag)
                    return copy;
                var dstr = 'M ';
                for (var i = 0; i < this.screenXY.length; i++) {
                    copy.screenXY[i].x = this.screenXY[i].x;
                    copy.screenXY[i].y = this.screenXY[i].y;
                    if (i > 0)
                        dstr += 'L ';
                    dstr += copy.screenXY[i].x + ' ' + copy.screenXY[i].y + ' ';
                }
                dstr += 'Z';
                copy.pathTag.setAttribute('d', dstr);
                return copy;
            };
            TangramFigure.prototype.MakeTangram = function (typenum, tsize) {
                ...
                switch (typenum) {
                    case 0:
                        ...
                        break;
                    case 1:
                        this.MakeFigure([10, 0, 10, 20, 0, 10], 20, tsize);
                        this.orgPosition.x = 10;
                        break;
                    case 2:
                        this.MakeFigure([0, 0, 10, 0, 15, 5, 5, 5], 20, tsize);
                        this.orgPosition.x = 5;
                        this.orgPosition.y = 15;
                        break;
                    case 3:
                        this.MakeFigure([0, 0, 10, 10, 0, 10], 20, tsize);
                        this.orgPosition.y = 10;
                        break;
                    case 4:
                        this.MakeFigure([0, 0, 5, 5, 0, 10], 20, tsize);
                        break;
                    case 5:
                        this.MakeFigure([5, 0, 10, 5, 5, 10, 0, 5], 20, tsize);
                        this.orgPosition.y = 5;
                        break;
                    case 6:
                        this.MakeFigure([5, 0, 10, 5, 0, 5], 20, tsize);
                        this.orgPosition.x = 5;
                        this.orgPosition.y = 10;
                        break;
                }
            };
            TangramFigure.prototype.GetDistance = function (a, b, p) {
                var ap = p.Sub(a);
                var ab = b.Sub(a);
                var ba = a.Sub(b);
                var bp = p.Sub(b);
                if (ap.DotProduct(ab) * ba.DotProduct(bp) >= 0) {
                    return Math.abs(ap.CrossProduct(ab) / a.DistanceFrom(b));
                }
                else {
                    return 100000;
                }
            };
            TangramFigure.prototype.GetCrossPoint = function (a, b, p) {
                var ab = b.Sub(a);
                var ap = p.Sub(a);
                return a.Add(ab.Mul(ab.DotProduct(ap) / ab.DotProduct(ab)));
            };
            TangramFigure.prototype.CheckSnap = function (x, y, orgTarget) {
                var pxy = this.svgCanvas.GetGroupPositionValue(this.groupTag);
                var mindistp = 20;
                var mindist = 15;
                var ok = false;
                var tx = 0, ty = 0;
                for (var i = 0; i < orgTarget.screenXY.length; i++) {
                    var P = new Point(x + orgTarget.screenXY[i].x, y + orgTarget.screenXY[i].y);
                    for (var j = -1; j < this.screenXY.length - 1; j++) {
                        var A = void 0, B = void 0;
                        if (j == -1 && this.screenXY.length > 2) {
                            A = new Point(this.screenXY[this.screenXY.length - 1].x + pxy.x, this.screenXY[this.screenXY.length - 1].y + pxy.y);
                            B = new Point(this.screenXY[0].x + pxy.x, this.screenXY[0].y + pxy.y);
                        }
                        else {
                            A = new Point(this.screenXY[j].x + pxy.x, this.screenXY[j].y + pxy.y);
                            B = new Point(this.screenXY[j + 1].x + pxy.x, this.screenXY[j + 1].y + pxy.y);
                        }
                        var dist = this.GetDistance(A, B, P);
                        if (dist < mindist) {
                            mindist = dist;
                            ok = true;
                            var crosspoint = this.GetCrossPoint(A, B, P);
                            tx = crosspoint.x - orgTarget.screenXY[i].x;
                            ty = crosspoint.y - orgTarget.screenXY[i].y;
                        }
                    }
                }
                for (var i = 0; i < orgTarget.screenXY.length; i++) {
                    var P = new Point(x + orgTarget.screenXY[i].x, y + orgTarget.screenXY[i].y);
                    for (var j = 0; j < this.screenXY.length; j++) {
                        var A = new Point(this.screenXY[j].x + pxy.x, this.screenXY[j].y + pxy.y);
                        var dist = A.DistanceFrom(P);
                        if (dist < mindistp) {
                            mindist = dist;
                            ok = true;
                            tx = A.x - orgTarget.screenXY[i].x;
                            ty = A.y - orgTarget.screenXY[i].y;
                        }
                    }
                }
                if (ok) {
                    return { x: tx, y: ty, distance: mindist };
                }
                else
                    return null;
            };
...
(function (upred) {
    ...
    (function (math) {
        var FigureType;
        (function (FigureType) {
            FigureType[FigureType["TANGRAM"] = 0] = "TANGRAM";
            FigureType[FigureType["GRAYED_BACK"] = 1] = "GRAYED_BACK";
        ...
        var Tangrams = (function () {
            ...
            Tangrams.prototype.HexToRGB = function (hex) {
                if (hex[0] == '#') {
            ...
            Tangrams.prototype.ChangeColor = function (item, colrstr) {
            ...
            Tangrams.prototype.InitPalette = function () {
                ...
                var templet = [
                    [
                        0, 33.561917064052,
                        5.00000000000006, 28.561917064052,
                        15, 28.561917064052,
                        15, 18.561917064052,
                        25, 8.56191706405197,
                        30, 3.56191706405196,
                        30, 0,
                        37.0710678118655, 7.07106781186545,
                        30, 7.07106781186545,
                        30, 13.561917064052,
                        35, 18.561917064052,
                        25.0000000000001, 18.561917064052,
                        30, 23.561917064052,
                        25.0000000000001, 28.561917064052,
                        30.0000000000001, 33.561917064052
                    ],
                    [
                        6.34824543303557, 38.2842712474619,
                        10.4903810567665, 34.142135623731,
                        10.4903810567665, 30.7350407972862,
                        3.41931324490104, 30.7350407972862,
                        10.4903810567665, 23.6639729854207,
                        10.4903810567665, 21.2132034355964,
                        3.41931324490104, 21.2132034355964,
                        3.41931324490104, 14.142135623731,
                        10.4903810567665, 14.142135623731,
                        1.83012701892213, 9.14213562373097,
                        0, 2.31200860480877,
                        8.66025403784433, 7.31200860480877,
                        10.4903810567665, 14.142135623731,
                        10.4903810567665, 0,
                        17.561448868632, 7.07106781186548,
                        10.4903810567665, 14.142135623731,
                        20.4903810567665, 24.142135623731,
                        20.4903810567665, 28.2842712474619,
                        25.4903810567665, 33.2842712474619,
                        20.4903810567665, 38.2842712474619
                    ],
                    [
                        7.07106781186549, 28.2842712474619,
                        7.07106781186549, 14.142135623731,
                        0, 7.07106781186548,
                        0, 0,
                        7.07106781186549, 7.07106781186548,
                        14.142135623731, 0,
                        14.142135623731, 7.07106781186548,
                        7.07106781186549, 14.142135623731,
                        11.2132034355965, 18.2842712474619,
                        24.142135623731, 18.2842712474619,
                        24.142135623731, 11.2132034355965,
                        31.2132034355965, 4.14213562373097,
                        38.2842712474619, 4.14213562373097,
                        31.2132034355965, 11.2132034355965,
                        31.2132034355965, 18.2842712474619,
                        21.2132034355964, 28.2842712474619
                    ],
                    [
                        0, 27.6776695296637,
                        7.07106781186548, 20.6066017177982,
                        7.07106781186548, 10.6066017177982,
                        0, 10.6066017177982,
                        0, 3.53553390593271,
                        7.07106781186548, 3.53553390593271,
                        13.1947921688235, 0,
                        16.7303260747562, 6.12372435695797,
                        7.07106781186548, 3.53553390593271,
                        7.07106781186548, 10.6066017177982,
                        27.0710678118655, 10.6066017177982,
                        31.2132034355965, 6.46446609406729,
                        31.2132034355965, 17.9624522875045,
                        36.2132034355965, 22.9624522875046,
                        31.2132034355965, 27.9624522875046,
                        31.2132034355965, 20.6066017177982,
                        14.142135623731, 20.6066017177982,
                        7.07106781186548, 27.6776695296637
                    ]
                ];
                var templetSize = [
                    [37.0710678118655, 33.561917064052],
                    [25.4903810567665, 38.2842712474619],
                ...
                for (var i = 0; i < 7; i++) {
                    ...
                    tng.RepositionIn();
                    tng.SetColor(colors[i * 3], colors[i * 3 + 1], colors[i * 3 + 2]);
                    tng.pathTag.addEventListener('pointerdown', this.OnDragStartNew.bind(this));
                    tangramBase.GetTag().appendChild(tng.groupTag);
                }
                tangramBase.GetTag().style.display = 'inline-block';
                ...
                for (var i = 0; i < templet.length; i++) {
                    ...
                    tng.RepositionIn();
                    tng.SetColor(colors[21], colors[22], colors[23]);
                    tng.pathTag.addEventListener('pointerdown', this.OnDragStartNew.bind(this));
                    tng.isAlwaysUnder = true;
                    templetBase.GetTag().appendChild(tng.groupTag);
                    templetBase.GetTag().style.display = 'inline-block';
                    templetBase.GetTag().style.margin = '8px 8px';
                    templetBase.SetSize(templetSize[i][0] / 20 * this.FIGURE_BASESIZE / TEMPLET_SIZE_DIV, templetSize[i][1] / 20 * this.FIGURE_BASESIZE / TEMPLET_SIZE_DIV);
                    this.paletteTag.appendChild(templetBase.GetTag());
                }
                window.addEventListener('pointermove', this.OnDragging.bind(this));
                window.addEventListener('pointerup', this.OnDragEnd.bind(this));
                this.svgCanvas.dragStarter.path = this.OnDragStartMove.bind(this);
            };
            Tangrams.prototype.OnDragStartNew = function (e) {
                var _a;
                if (!e.currentTarget)
                    return;
                ...
                var orgTangram = e.currentTarget.FINST;
                var newTangram = orgTangram.Duplicate(this.WORKSIZE, true);
                newTangram.MakeRotater(this.WORKSIZE);
                this.xyOrg.x = xy.left - newTangram.allWidth / 2 + orgTangram.allWidth / 2;
                this.xyOrg.y = xy.top - newTangram.allHeight / 2 + orgTangram.allHeight / 2;
                this.xyBefore.x = e.pageX;
                this.xyBefore.y = e.pageY;
            ...
            Tangrams.prototype.OnDragStartMove = function (e) {
                var _a, _b;
                if (!e.target.FINST)
                    return false;
            ...
            Tangrams.prototype.OnDragging = function (e) {
                if (this.curDragging) {
                    if (this.dragMode == DragMode.MOVE) {
                        ...
                        var nex = this.xyOrg.x + dx;
                        var ney = this.xyOrg.y + dy;
                        var distance = 1001;
                        for (var i = 0; i < this.tangrams.length; i++) {
                            if (this.curDragging == this.tangrams[i])
                                continue;
                            var p = this.tangrams[i].CheckSnap(nex, ney, this.curDragging);
                            if (p) {
                        ...
                        this.svgCanvas.SetGroupPositionValue(this.curDragging.groupTag, nex, ney);
            ...
            Tangrams.prototype.OnDragEnd = function (e) {
                var _a, _b;
                if (this.dragMode == DragMode.MOVE) {
                    var dx = this.xyNow.x - this.xyBefore.x;
                    var dy = this.xyNow.y - this.xyBefore.y;
            ...
            Tangrams.prototype.PrintXY = function (t) {
                var _a;
                var p = (_a = this.svgCanvas) === null || _a === void 0 ? void 0 : _a.GetGroupPositionValue(t.groupTag);
                for (var i = 0; i < t.screenXY.length; i++) {
                    console.log("Point[" + i + "] " + (t.screenXY[i].x + p.x) + ", " + (t.screenXY[i].y + p.y));
                }
            };
            Tangrams.prototype.RemoveByGroupTag = function (grouptag) {
                var n = -1;
                for (var i = 0; i < this.tangrams.length; i++) {
                    if (this.tangrams[i].groupTag == grouptag) {
                        n = i;
                        break;
                    }
                }
                if (n >= 0)
                    this.tangrams.splice(n, 1);
            };
            Tangrams.prototype.AssignHandlers = function (group, x, y) {
                ...
                this.tangrams.push(group);
            };
            Tangrams.prototype.ShowHighlighted = function (group, flag) {
                if (group) {
                    if (flag) {
                        group.groupTag.firstChild.setAttribute('filter', 'url(#filterOutline)');
                    }
                    else {
                        group.groupTag.firstChild.setAttribute('filter', '');
                        group.ShowRotater(false);
                    }
                }
            };
            Tangrams.prototype.SetSelectionBy = function (x, y, w, h) {
                var _a;
                for (var i = 0; i < this.tangrams.length; i++) {
                    var ar = this.tangrams[i].groupTag.getBoundingClientRect();
                    if (!(x > ar.right ||
                        x + w < ar.left ||
                        y > ar.bottom ||
                        y + h < ar.top)) {
            ...
            Tangrams.prototype.Rotate = function (group, degree) {
                group.Rotate(degree / 6);
            };
            Tangrams.prototype.Delete = function (targetGroup) {
                if (targetGroup != null && this.tangrams.indexOf(targetGroup) >= 0) {
                    this.svgCanvas.RemoveGroup(targetGroup.groupTag);
                    this.tangrams.splice(this.tangrams.indexOf(targetGroup), 1);
                }
            };
            Tangrams.prototype.Duplicate = function (targetGroup) {
                var _a, _b;
                if (targetGroup != null) {
            ...
            Tangrams.prototype.RestoreInitialState = function () {
                ...
                for (var i = 0; i < this.tangrams.length; i++) {
                    this.svgCanvas.RemoveGroup(this.tangrams[i].groupTag);
                }
                this.tangrams = [];
            };
            Tangrams.prototype.EnableInteraction = function () {
                this.svgCanvas.MoveToFront();
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:750;');
            };
            Tangrams.prototype.DisableInteraction = function () {
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:400;');
            };
