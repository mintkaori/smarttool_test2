...
(function (upred) {
    ...
    (function (math) {
        ...
        var ClockUnit = (function () {
            function ClockUnit(canvas) {
                ...
                this.bgGroupTag = this.MakeTagStyled(this.groupTag, 'g', {}, { touchAction: 'none', pointerEvents: 'none', userSelect: 'none' });
                this.groupDigital = this.MakeTagStyled(this.groupTag, 'g', {}, { touchAction: 'none', pointerEvents: 'none', userSelect: 'none' });
                this.MakeTagStyled(this.groupDigital, 'image', { href: './asset/digitalwatch.png', width: '300', height: '144', x: '0', y: '0' }, { touchAction: 'none', userSelect: 'none', pointerEvents: 'none' });
                this.digitalTextTag = this.MakeTagStyled(this.groupDigital, 'text', { textLength: '260', x: '20', y: '20', 'dominant-baseline': 'hanging' }, { strokeWidth: '0', fill: 'black', fontSize: '100', fontFamily: 'sans-serif', fontWeight: 'bold', touchAction: 'none', userSelect: 'none', pointerEvents: 'none' });
                this.digitalTextTag.textContent = '12:00';
                this.groupDigital.style.display = 'none';
            }
            ClockUnit.prototype.MakeClock = function (useGear, unitW, donotassign) {
                if (donotassign === void 0) { donotassign = false; }
                this.unitW = unitW;
                this.useGear = useGear;
                var R = unitW / 2;
                this.moveAreaTag = this.MakeTagStyled(this.groupTag, 'circle', { cx: '0', cy: '0', r: (R).toString() }, { strokeWidth: '0', fill: 'transparent', touchAction: 'none', pointerEvents: 'all', userSelect: 'none' });
                this.moveAreaTag.FINST = this;
                this.MakeTagStyled(this.bgGroupTag, 'circle', { cx: '2', cy: '4', r: (R + 1).toString() }, { strokeWidth: '0', touchAction: 'none', pointerEvents: 'none', userSelect: 'none', filter: 'brightness(0.5)' });
                this.MakeTagStyled(this.bgGroupTag, 'circle', { cx: '0', cy: '0', r: (R).toString() }, { fill: 'url(#OutterRing)', strokeWidth: '1', touchAction: 'none', pointerEvents: 'none', userSelect: 'none' });
                this.MakeTagStyled(this.bgGroupTag, 'circle', { cx: '0', cy: '0', r: (R * 0.9).toString() }, { fill: 'url(#InnerCirlceBG)', strokeWidth: '0', touchAction: 'none', pointerEvents: 'none', userSelect: 'none' });
                var deg = 0;
                for (var i = 0; i < 60; i++) {
                    var inner = this.GetPointAt(new Point(0, i % 5 == 0 ? R * 0.82 : R * 0.85), deg);
                    var outter = this.GetPointAt(new Point(0, R * 0.9), deg);
                    deg += 6;
                    this.MakeTagStyled(this.bgGroupTag, 'line', { x1: inner.x.toString(), y1: inner.y.toString(), x2: outter.x.toString(), y2: outter.y.toString() }, { lineCap: 'butt', strokeWidth: i % 5 == 0 ? '2' : '1', stroke: 'black', touchAction: 'none', pointerEvents: 'none', userSelect: 'none' });
                }
                deg = 0;
                for (var i = 0; i < 12; i++) {
                    var p = this.GetPointAt(new Point(0, -R * 0.72), deg);
                    deg += 30;
                    var text = this.MakeTagStyled(this.bgGroupTag, 'text', { x: p.x.toString(), y: p.y.toString(), 'dominant-baseline': 'middle', 'text-anchor': 'middle' }, { fontFamily: 'sans-serif', fontSize: (R * 0.18).toString(), strokeWidth: '0', fill: 'black', touchAction: 'none', pointerEvents: 'none', userSelect: 'none' });
                    text.textContent = i == 0 ? '12' : i.toString();
                }
                if (useGear) {
                    var dstr = 'M ';
                    for (var i = 0; i < 60; i++) {
                        var inner = this.GetPointAt(new Point(0, R * 0.18), deg);
                        var outter = this.GetPointAt(new Point(0, R * 0.17), deg);
                        deg += 6;
                        if (i > 0)
                            dstr += ' L ';
                        if (i % 2 == 0) {
                            var outter2 = this.GetPointAt(new Point(0, R * 0.17), deg);
                            dstr += inner.x + ' ' + inner.y + ' L ' + outter.x + ' ' + outter.y + ' L ' + outter2.x + ' ' + outter2.y;
                        }
                        else {
                            var inner2 = this.GetPointAt(new Point(0, R * 0.18), deg);
                            dstr += outter.x + ' ' + outter.y + ' L ' + inner.x + ' ' + inner.y + ' L ' + inner2.x + ' ' + inner2.y;
                        }
                    }
                    dstr += ' Z';
                    this.gearTag = this.MakeTagStyled(this.bgGroupTag, 'path', { d: dstr }, { strokeWidth: '1', stroke: 'rgb(168, 168, 168)', fill: 'url(#GearGrad)', touchAction: 'none', pointerEvents: 'none', userSelect: 'none' });
                    var dstr2 = 'M ';
                    var dd = R * 0.164;
                    deg = 0;
                    for (var i = 0; i < 20; i++) {
                        var inner = this.GetPointAt(new Point(0, R * 0.06), deg);
                        var outter = this.GetPointAt(new Point(0, R * 0.05), deg);
                        deg += 18;
                        if (i > 0)
                            dstr2 += ' L ';
                        if (i % 2 == 0) {
                            var outter2 = this.GetPointAt(new Point(0, R * 0.05), deg);
                            dstr2 += inner.x + ' ' + inner.y + ' L ' + outter.x + ' ' + outter.y + ' L ' + outter2.x + ' ' + outter2.y;
                        }
                        else {
                            var inner2 = this.GetPointAt(new Point(0, R * 0.06), deg);
                            dstr2 += outter.x + ' ' + outter.y + ' L ' + inner.x + ' ' + inner.y + ' L ' + inner2.x + ' ' + inner2.y;
                        }
                    }
                    dstr2 += ' Z';
                    this.gear2Tag = this.MakeTagStyled(this.bgGroupTag, 'path', { d: dstr2 }, { strokeWidth: '1', stroke: 'rgb(168, 168, 168)', fill: 'url(#GearGrad)', transform: 'translate(' + dd + 'px,' + dd + 'px)', touchAction: 'none', pointerEvents: 'none', userSelect: 'none' });
                }
                var hThick = R * 0.12;
                var hRadius = R * 0.45;
                var mThick = R * 0.1;
                ...
                var thin = 0.4;
                var hourXY = [
                    'M', '0', (hThick * thin).toString(),
                    'Q', (-hThick * thin).toString(), (hThick * thin).toString(), (-hThick * thin).toString(), '0',
                    'L', (-hThick * thin).toString(), (-hRadius + hThick * 1.4).toString(),
                    'L', (-hThick).toString(), (-hRadius + hThick * 1.6).toString(),
                    'L', '0', (-hRadius).toString(),
                    'L', (hThick).toString(), (-hRadius + hThick * 1.6).toString(),
                    'L', (hThick * thin).toString(), (-hRadius + hThick * 1.4).toString(),
                    'L', (hThick * thin).toString(), '0',
                    'Q', (hThick * thin).toString(), (hThick * thin).toString(), '0', (hThick * thin).toString(),
                    'Z'
                ];
                var minXY = [
                    'M', '0', (mThick * thin).toString(),
                    'Q', (-mThick * thin).toString(), (mThick * thin).toString(), (-mThick * thin).toString(), '0',
                    'L', (-mThick * thin).toString(), (-mRadius + mThick * 1.4).toString(),
                    'L', (-mThick).toString(), (-mRadius + mThick * 1.6).toString(),
                    'L', '0', (-mRadius).toString(),
                    'L', (mThick).toString(), (-mRadius + mThick * 1.6).toString(),
                    'L', (mThick * thin).toString(), (-mRadius + mThick * 1.4).toString(),
                    'L', (mThick * thin).toString(), '0',
                    'Q', (mThick * thin).toString(), (mThick * thin).toString(), '0', (mThick * thin).toString(),
                    'Z'
                ];
                this.hourTag = this.MakeTagStyled(this.groupTag, 'path', { d: hourXY.join(' ') }, { strokeWidth: '2', fill: '#27AE60', stroke: '#229954', touchAction: 'none', userSelect: 'none', pointerEvents: 'visiblePainted' });
                this.minuteTag = this.MakeTagStyled(this.groupTag, 'path', { d: minXY.join(' ') }, { strokeWidth: '2', fill: '#E74C3C', stroke: '#CB4335', touchAction: 'none', userSelect: 'none', pointerEvents: 'visiblePainted' });
                this.MakeTagStyled(this.groupTag, 'circle', { cx: '0', cy: '0', r: '4' }, { strokeWidth: '0', fill: 'black', touchAction: 'none', pointerEvents: 'none', userSelect: 'none' });
                this.hour = 12;
                this.minute = 0;
                if (!donotassign) {
                    this.hourTag.onpointerdown = this.OnHourMoveStart.bind(this);
                    this.minuteTag.onpointerdown = this.OnMinuteMoveStart.bind(this);
                }
                this.groupDigital.style.transform = 'translate(-150px,' + (R + 20) + 'px)';
            };
            ClockUnit.prototype.OnHourMoveStart = function (e) {
                var _a;
                var xy = this.svgCanvas.GetGroupPositionValue(this.groupTag);
                this.orgX = xy.x;
                this.orgY = xy.y;
                upred.ui.mathModule.dragMode = upred.math.DragMode.ROTATE_HOUR;
                upred.ui.mathModule.curDragging = this;
                (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.SelectItemOnlyOne(this);
                e.preventDefault();
                e.stopPropagation();
            };
            ClockUnit.prototype.OnMinuteMoveStart = function (e) {
                var _a;
                var xy = this.svgCanvas.GetGroupPositionValue(this.groupTag);
                this.orgX = xy.x;
                this.orgY = xy.y;
                upred.ui.mathModule.dragMode = upred.math.DragMode.ROTATE_MINUTE;
                upred.ui.mathModule.curDragging = this;
                (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.SelectItemOnlyOne(this);
                e.preventDefault();
                e.stopPropagation();
            };
            ClockUnit.prototype.UpdateHour = function (deg) {
                deg %= 360;
                if (deg < 0)
                    deg += 360;
                this.hourDegree = deg;
                this.hourTag.style.transform = 'rotate(' + deg + 'deg)';
                if (this.gearTag)
                    this.gearTag.style.transform = 'rotate(' + deg + 'deg)';
                if (this.gear2Tag)
                    this.gear2Tag.style.transform = 'translate(' + (this.unitW / 2 * 0.164) + 'px,' + (this.unitW / 2 * 0.164) + 'px) rotate(' + (deg * -3) + 'deg)';
                this.hour = Math.floor(deg / 30);
                if (this.hour == 0)
                    this.hour = 12;
                this.digitalTextTag.textContent = this.hour + ':' + (this.minute < 10 ? ('0' + this.minute) : this.minute);
                if (this.hour < 10) {
                    this.digitalTextTag.setAttribute('x', '80');
                    this.digitalTextTag.setAttribute('textLength', '200');
                }
                else {
                    this.digitalTextTag.setAttribute('x', '20');
                    this.digitalTextTag.setAttribute('textLength', '260');
                }
            };
            ClockUnit.prototype.UpdateMinute = function (deg) {
                deg %= 360;
                if (deg < 0)
                    deg += 360;
                this.minDegree = deg;
                this.minuteTag.style.transform = 'rotate(' + deg + 'deg)';
                this.minute = Math.floor(deg / 6);
                this.digitalTextTag.textContent = this.hour + ':' + (this.minute < 10 ? ('0' + this.minute) : this.minute);
                if (this.hour < 10) {
                    this.digitalTextTag.setAttribute('x', '80');
                    this.digitalTextTag.setAttribute('textLength', '200');
            ...
            ClockUnit.prototype.MoveHour = function (e) {
                var deg = Math.atan2(e.pageY - this.orgY, e.pageX - this.orgX) * 180 / Math.PI + 90;
                if (deg < 0)
                    deg += 360;
                deg %= 360;
                if (this.useGear) {
                    ...
                    var mdeg = ((deg % 30) * 12);
                    this.UpdateMinute(mdeg);
                }
                this.UpdateHour(deg);
            };
            ClockUnit.prototype.MoveMinute = function (e) {
                var deg = Math.atan2(e.pageY - this.orgY, e.pageX - this.orgX) * 180 / Math.PI + 90;
                if (deg < 0)
                    deg += 360;
                deg %= 360;
                if (this.useGear) {
                    deg = Math.floor(deg / 6) * 6;
                    var hdeg = (this.hour == 12 ? 0 : this.hour) * 30;
                    hdeg += 30 * (deg / 360);
                    if (this.minDegree > 270 && deg < 90)
                        hdeg += 30;
                    else if (this.minDegree < 90 && deg > 270)
                        hdeg -= 30;
                    this.UpdateHour(hdeg);
                }
                this.UpdateMinute(deg);
            };
            ClockUnit.prototype.GetPointAt = function (p, deg) {
                var rad = deg * Math.PI / 180;
                return new Point(p.x * Math.cos(rad) - p.y * Math.sin(rad), p.x * Math.sin(rad) + p.y * Math.cos(rad));
            };
            ClockUnit.prototype.Duplicate = function () {
                var pc = new ClockUnit(this.svgCanvas);
                pc.unitW = this.unitW;
                pc.MakeClock(this.useGear, this.unitW);
                pc.SetColor(this.colorValue.r, this.colorValue.g, this.colorValue.b);
                return pc;
            };
            ClockUnit.prototype.SetColor = function (r, g, b) {
                this.colorValue.r = r;
                this.colorValue.g = g;
                this.colorValue.b = b;
                this.groupTag.style.stroke = this.MakeRGB(r, g, b, 0.5);
                this.groupTag.style.fill = this.MakeRGB(r, g, b, 1);
                var grad = document.getElementById('OutterRing').cloneNode(true);
                var gparent = document.getElementById('OutterRing').parentNode;
                var nid = 'OutterRing' + ClockUnit.G_SN.toString();
                grad.id = nid;
                ClockUnit.G_SN++;
                grad.children[0].style.stopColor = this.MakeRGB(r, g, b, 0.85);
                grad.children[1].style.stopColor = this.MakeRGB(r, g, b, 1);
                grad.children[2].style.stopColor = this.MakeRGB(r, g, b, 1);
                grad.children[3].style.stopColor = this.MakeRGB(r, g, b, 1.3);
                gparent.appendChild(grad);
                this.bgGroupTag.children[1].style.fill = 'url(#' + nid + ')';
            };
            ClockUnit.prototype.MakeRGB = function (r, g, b, m) {
                r *= m;
                if (r < 0)
                    r = 0;
                if (r > 255)
            ...
            ClockUnit.prototype.MakeTagStyled = function (parent, tagname, attr, style) {
            ...
            ClockUnit.prototype.Highlight = function (flag) {
                if (flag) {
                    this.groupTag.setAttribute('filter', 'url(#filterOutline)');
                }
                else {
                    this.groupTag.setAttribute('filter', '');
                }
            };
            ClockUnit.G_SN = 0;
            return ClockUnit;
        }());
        math.ClockUnit = ClockUnit;
...
(function (upred) {
    ...
    (function (math) {
        ...
        (function (DragMode) {
            ...
            DragMode[DragMode["ROTATE_MINUTE"] = 2] = "ROTATE_MINUTE";
            DragMode[DragMode["ROTATE_HOUR"] = 3] = "ROTATE_HOUR";
        ...
        var Clocks = (function () {
            function Clocks() {
                ...
                this.UNIT_SIZE = 60;
                this.digitalButton = {
                    img: './asset/icon_digitalwatch.png',
                    txt: '디지털',
                    handler: this.OnClickDigital.bind(this),
                    checkHandler: this.CheckDigital.bind(this)
                };
            }
            Clocks.prototype.HexToRGB = function (hex) {
                if (hex[0] == '#') {
            ...
            Clocks.prototype.ChangeColor = function (item, colrstr) {
                var color = this.HexToRGB(colrstr);
                if (item.SetColor) {
                    item.SetColor(color.r, color.g, color.b);
                }
            };
            Clocks.prototype.OnDragStartNew = function (e) {
                var _a;
                if (!e.currentTarget)
                    return;
                if (!e.currentTarget)
                    return;
                if (!(e.currentTarget.FINST))
                    return;
                ...
                var org = e.currentTarget.FINST;
                var newpc = org.Duplicate();
                this.xyOrg.x = xy.left;
                this.xyOrg.y = xy.top;
                this.xyBefore.x = e.pageX;
                this.xyBefore.y = e.pageY;
                this.AssignHandlerAndRegister(newpc, this.xyOrg.x, this.xyOrg.y);
                (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.SelectItemOnlyOne(newpc);
                this.dragMode = DragMode.MOVE;
                this.curDragging = newpc;
                this.curDragging.groupTag.style.opacity = '0.9';
            };
            Clocks.prototype.OnDragStartMove = function (e) {
                var _a;
                var usetag = e.target;
                if (!usetag)
                    return;
                if (this.dragMode == DragMode.NONE) {
                    ...
                    for (var i = 0; i < this.allObjs.length; i++) {
                        if (this.allObjs[i].type == 'clock' && this.allObjs[i].moveAreaTag == usetag) {
                    ...
                    if (!this.curDragging) {
                        ...
                        this.dragMode = DragMode.MOVE;
                        var gxy = this.svgCanvas.GetGroupPositionValue(this.curDragging.groupTag);
                        this.xyOrg.x = gxy.x;
                        this.xyOrg.y = gxy.y;
                        e.preventDefault();
                    }
            ...
            Clocks.prototype.OnDragging = function (e) {
                if (!this.curDragging)
                    return;
                var dx = e.pageX - this.xyBefore.x;
                var dy = e.pageY - this.xyBefore.y;
                if (this.curDragging.type != 'clock')
                    return;
                switch (this.dragMode) {
                    case DragMode.MOVE:
                        {
                            this.curDragging.groupTag.style.opacity = '0.7';
                            this.svgCanvas.SetGroupPositionValue(this.curDragging.groupTag, this.xyOrg.x + dx, this.xyOrg.y + dy);
                        }
                        break;
                    case DragMode.ROTATE_HOUR:
                        {
                            this.curDragging.MoveHour(e);
                        }
                        break;
                    case DragMode.ROTATE_MINUTE:
                        {
                            this.curDragging.MoveMinute(e);
                        }
                        break;
                }
            };
            Clocks.prototype.OnDragEnd = function (e) {
                var _a, _b;
                if (!this.curDragging)
                    return;
                ...
                if (dx * dx + dy * dy < 32) {
                    ...
                    if (e.pageX < pPos.right && e.pageY > pPos.top) {
                        var ind = this.allObjs.indexOf(this.curDragging);
                        if (ind >= 0)
                            this.allObjs.splice(ind, 1);
            ...
            Clocks.prototype.AssignHandlerAndRegister = function (tline, x, y) {
                ...
                this.svgCanvas.GetTag().appendChild(tline.groupTag);
                this.allObjs.push(tline);
            };
            Clocks.prototype.ShowHighlighted = function (group, flag) {
                if (group) {
                    if (group.Highlight)
                        group.Highlight(flag);
                }
            };
            Clocks.prototype.SetSelectionBy = function (x, y, w, h) {
                var _a;
                for (var i = 0; i < this.allObjs.length; i++) {
            ...
            Clocks.prototype.Rotate = function (group, degree) {
            };
            Clocks.prototype.Delete = function (targetGroup) {
            ...
            Clocks.prototype.Duplicate = function (targetGroup) {
                var _a, _b;
                var copy = targetGroup.Duplicate();
                var xy = this.svgCanvas.GetGroupPositionValue(targetGroup.groupTag);
                this.AssignHandlerAndRegister(copy, xy.x + this.UNIT_SIZE, xy.y);
                (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.SelectItemAdd(targetGroup);
                (_b = upred.ui.uiModule) === null || _b === void 0 ? void 0 : _b.duplicated.push(copy);
            };
            Clocks.prototype.RestoreInitialState = function () {
            ...
            Clocks.prototype.EnableInteraction = function () {
                this.svgCanvas.MoveToFront();
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:750;');
            };
            Clocks.prototype.DisableInteraction = function () {
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:400;');
            };
            Clocks.prototype.InitPalette = function () {
                ...
                this.UNIT_SIZE = Math.min(Math.max(RW, RH) * 0.45, Math.min(RW, RH)) * 0.8;
                var colors = [
                    231, 76, 60,
                    230, 126, 34
                ];
                var clockA = new upred.ui.SVGUnit();
                var tng = new math.ClockUnit(this.svgCanvas);
                tng.MakeClock(true, this.UNIT_SIZE, true);
                tng.SetColor(colors[0], colors[1], colors[2]);
                tng.moveAreaTag.onpointerdown = this.OnDragStartNew.bind(this);
                clockA.GetTag().appendChild(tng.groupTag);
                clockA.GetTag().style.display = 'inline-block';
                clockA.GetTag().style.margin = '8px 8px';
                clockA.SetViewBox(-this.UNIT_SIZE / 2 - 2, -this.UNIT_SIZE / 2 - 2, this.UNIT_SIZE + 6, this.UNIT_SIZE + 10);
                clockA.SetSize(this.UNIT_SIZE / 2, this.UNIT_SIZE / 2);
                this.paletteTag.appendChild(clockA.GetTag());
                var clockB = new upred.ui.SVGUnit();
                var tng2 = new math.ClockUnit(this.svgCanvas);
                tng2.MakeClock(false, this.UNIT_SIZE, true);
                tng2.SetColor(colors[3], colors[4], colors[5]);
                tng2.moveAreaTag.onpointerdown = this.OnDragStartNew.bind(this);
                clockB.GetTag().appendChild(tng2.groupTag);
                clockB.GetTag().style.display = 'inline-block';
                clockB.GetTag().style.margin = '8px 8px';
                clockB.SetViewBox(-this.UNIT_SIZE / 2 - 2, -this.UNIT_SIZE / 2 - 2, this.UNIT_SIZE + 6, this.UNIT_SIZE + 10);
                clockB.SetSize(this.UNIT_SIZE / 2, this.UNIT_SIZE / 2);
                this.paletteTag.appendChild(clockB.GetTag());
                window.addEventListener('pointermove', this.OnDragging.bind(this));
                window.addEventListener('pointerup', this.OnDragEnd.bind(this));
                this.svgCanvas.dragStarter.circle = this.OnDragStartMove.bind(this);
            };
            Clocks.prototype.OnClickDigital = function () {
                var _a, _b;
                for (var i = 0; i < upred.ui.uiModule.selectedItems.length; i++) {
                    if (upred.ui.uiModule.selectedItems[i].type && upred.ui.uiModule.selectedItems[i].type == 'clock') {
                        var cu = upred.ui.uiModule.selectedItems[i];
                        if (cu.groupDigital.style.display == 'none') {
                            cu.groupDigital.style.display = 'block';
                            (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.SelectButton(this.digitalButton);
                        }
                        else {
                            cu.groupDigital.style.display = 'none';
                            (_b = upred.ui.uiModule) === null || _b === void 0 ? void 0 : _b.SelectButton(null);
                        }
                    }
                }
            };
            Clocks.prototype.CheckDigital = function () {
                var _a, _b;
                for (var i = 0; i < upred.ui.uiModule.selectedItems.length; i++) {
                    if (upred.ui.uiModule.selectedItems[i].type && upred.ui.uiModule.selectedItems[i].type == 'clock') {
                        var cu = upred.ui.uiModule.selectedItems[i];
                        if (cu.groupDigital.style.display == 'none') {
                            (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.SelectButton(null);
                        }
                        else {
                            (_b = upred.ui.uiModule) === null || _b === void 0 ? void 0 : _b.SelectButton(this.digitalButton);
                        }
                    }
                }
                return true;
            ...
            Clocks.prototype.Init = function () {
                ...
                this.svgCanvas = new upred.ui.SVGCanvas();
                this.InitPalette();
                (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.DisableDuplication();
                (_b = upred.ui.uiModule) === null || _b === void 0 ? void 0 : _b.DisableRotation();
                (_c = upred.ui.uiModule) === null || _c === void 0 ? void 0 : _c.RemoveMainButton(upred.ui.uiModule.uiButtons.select);
                (_d = upred.ui.uiModule) === null || _d === void 0 ? void 0 : _d.AddCustomModifier('digital', this.digitalButton);
                window.addEventListener('pointermove', this.OnDragging.bind(this));
                window.addEventListener('pointerup', this.OnDragEnd.bind(this));
