...
(function (upred) {
    var math;
    (function (math) {
        var RackRow = (function () {
            function RackRow(canvas, wsize, hsize) {
                ...
                this.rectTag = this.MakeTagStyled(this.groupTag, 'rect', { x: '0', y: (-hsize / 2).toString(), fill: 'url(#rowlinefill)', width: wsize, height: hsize }, { strokeWidth: '0', touchAction: 'none', pointerEvents: 'none', userSelect: 'none' });
                this.rowWidth = wsize;
                this.rowHeight = hsize;
                var bR = upred.ui.mathModule.bidSizeR;
                for (var i = 9; i >= 5; i--) {
                    var bid = this.MakeTagStyled(this.groupTag, 'use', { href: '#bidRed', x: wsize - (bR * 2 * i + bR), y: '0' }, { touchAction: 'none', pointerEvents: 'visiblePainted', userSelect: 'none', cursor: 'grab' });
                    this.bids.push(bid);
                }
                for (var i = 4; i >= 0; i--) {
                    var bid = this.MakeTagStyled(this.groupTag, 'use', { href: '#bidWhite', x: wsize - (bR * 2 * i + bR), y: '0' }, { touchAction: 'none', pointerEvents: 'visiblePainted', userSelect: 'none', cursor: 'grab' });
                    this.bids.push(bid);
                }
                for (var i = 0; i < this.bids.length; i++) {
                    this.bids[i].onpointerdown = this.OnMoveStart.bind(this);
                }
            }
            RackRow.prototype.OnMoveStart = function (e) {
                var bid = e.currentTarget;
                upred.ui.mathModule.dragMode = math.DragMode.MOVE;
                upred.ui.mathModule.curDragging = this;
                upred.ui.mathModule.curDraggingBid = bid;
                upred.ui.mathModule.xBefore = e.pageX;
                upred.ui.mathModule.xOrg = parseFloat(bid.getAttribute('x'));
                e.preventDefault();
            };
            RackRow.prototype.MoveBid = function (b, nx) {
                var index = this.bids.indexOf(b);
                if (index < 0)
                    return;
                var radius = upred.ui.mathModule.bidSizeR;
                var minX = index * (radius * 2) + radius;
                var maxX = this.rowWidth - (this.bids.length - index) * (radius * 2) + radius;
                if (nx < minX)
                    nx = minX;
                if (nx > maxX)
                    nx = maxX;
                b.setAttribute('x', nx.toString());
                for (var i = index - 1, cx = nx - radius * 2; i >= 0; i--, cx -= radius * 2) {
                    var x = parseFloat(this.bids[i].getAttribute('x'));
                    if (x > cx)
                        this.bids[i].setAttribute('x', cx.toString());
                }
                for (var i = index + 1, cx = nx + radius * 2; i < this.bids.length; i++, cx += radius * 2) {
                    var x = parseFloat(this.bids[i].getAttribute('x'));
                    if (x < cx)
                        this.bids[i].setAttribute('x', cx.toString());
                }
            ...
            RackRow.prototype.MakeRod = function (unitW, lengthCount) {
                this.unitW = unitW;
                this.lengthCount = lengthCount;
                this.rectTag.setAttribute('width', (unitW * lengthCount).toString());
                this.rectTag.setAttribute('height', unitW.toString());
            };
            RackRow.prototype.Duplicate = function () {
                var pc = new RackRow(this.svgCanvas, this.rowWidth, this.rowHeight);
                pc.unitW = this.unitW;
                pc.lengthCount = this.lengthCount;
                pc.MakeRod(this.unitW, this.lengthCount);
                pc.SetColor(this.colorValue.r, this.colorValue.g, this.colorValue.b);
                return pc;
            };
            RackRow.prototype.SetColor = function (r, g, b) {
                this.colorValue.r = r;
                this.colorValue.g = g;
                this.colorValue.b = b;
            ...
            RackRow.prototype.MakeRGB = function (r, g, b, m) {
                r *= m;
                if (r < 0)
                    r = 0;
                if (r > 255)
            ...
            RackRow.prototype.MakeTagStyled = function (parent, tagname, attr, style) {
            ...
            RackRow.prototype.Highlight = function (flag) {
                if (flag) {
...
(function (upred) {
    ...
    (function (math) {
        ...
        var NumberRack = (function () {
            ...
            NumberRack.prototype.HexToRGB = function (hex) {
                if (hex[0] == '#') {
            ...
            NumberRack.prototype.ChangeColor = function (item, colrstr) {
                var color = this.HexToRGB(colrstr);
                if (item.SetColor) {
                    item.SetColor(color.r, color.g, color.b);
                }
            };
            NumberRack.prototype.OnDragging = function (e) {
                if (this.dragMode != DragMode.MOVE)
                    return;
                if (!this.curDragging || !this.curDraggingBid)
                    return;
                var dx = e.pageX - this.xBefore;
                this.curDragging.MoveBid(this.curDraggingBid, this.xOrg + dx);
                this.lastMoveTime = new Date().getTime();
            };
            NumberRack.prototype.OnDragEnd = function (e) {
                if (!this.curDragging)
                    return;
                ...
                this.lastMoveTime = new Date().getTime();
                var dx = e.pageX - this.xBefore;
                this.speed = dx / elapsed * 0.5;
                this.dragMode = DragMode.NONE;
                e.preventDefault();
            };
            NumberRack.prototype.MoveContinue = function () {
                if (this.dragMode == DragMode.MOVE || !this.curDragging || !this.curDraggingBid) {
                    return;
                }
                var x = parseFloat(this.curDraggingBid.getAttribute('x'));
                this.speed *= 0.8;
                this.curDragging.MoveBid(this.curDraggingBid, x + this.speed);
                if (Math.abs(this.speed) < 1) {
                    this.speed = 0;
                    this.curDragging = undefined;
                    this.curDraggingBid = undefined;
                }
            };
            NumberRack.prototype.AssignHandlerAndRegister = function (tline, x, y) {
                ...
                this.allRows.push(tline);
            };
            NumberRack.prototype.ShowHighlighted = function (group, flag) {
                if (group) {
                    if (group.Highlight)
                        group.Highlight(flag);
                }
            };
            NumberRack.prototype.SetSelectionBy = function (x, y, w, h) {
                var _a;
                for (var i = 0; i < this.allRows.length; i++) {
                    if (this.allRows[i].type == 'board')
                        continue;
                    var ar = this.allRows[i].groupTag.getBoundingClientRect();
                    if (!(x > ar.right ||
                        x + w < ar.left ||
                        y > ar.bottom ||
                        y + h < ar.top)) {
                        (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.SelectItemAdd(this.allRows[i]);
                    }
                }
            };
            NumberRack.prototype.Rotate = function (group, degree) {
            };
            NumberRack.prototype.Delete = function (targetGroup) {
            };
            NumberRack.prototype.Duplicate = function (targetGroup) {
                var _a, _b;
                var copy = targetGroup.Duplicate();
                var xy = this.svgCanvas.GetGroupPositionValue(targetGroup.groupTag);
                this.AssignHandlerAndRegister(copy, xy.x + this.UNIT_SIZE * copy.lengthCount, xy.y);
            ...
            NumberRack.prototype.RestoreInitialState = function () {
                location.reload();
            };
            NumberRack.prototype.EnableInteraction = function () {
                this.svgCanvas.MoveToFront();
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:750;');
            };
            NumberRack.prototype.DisableInteraction = function () {
                this.svgCanvas.GetTag().setAttribute('style', 'position:absolute; left:0px; top:0px; z-index:400;');
            };
            NumberRack.prototype.InitBoard = function () {
            };
            NumberRack.prototype.MakeBid = function (ids, radius, fill) {
                var bid = new upred.ui.SVGSymbol(ids);
                bid.AddCircle(0, 0, radius, 'url(#' + fill + ')', '#000000', 0);
                bid.AddCircle(0, -radius * 0.5, radius * 0.35, 'url(#bidHighlight)', '#000000', 0);
                bid.SetSize((radius * 2).toString(), (radius * 2).toString());
                bid.symbolTag.style.overflow = 'visible';
                return bid;
            };
            NumberRack.prototype.ArrangeRows = function () {
                var fLeft = document.getElementById('FRAME_LEFT').getBoundingClientRect();
                var cellH = this.hSize / this.allRows.length;
                var midN = this.allRows.length / 2;
                if (this.allRows.length % 2 > 0) {
                    this.svgCanvas.SetGroupPositionValue(this.allRows[Math.floor(midN)].groupTag, fLeft.width, this.hSize / 2);
                }
                if (this.allRows.length >= 2) {
                    var ydelta = this.bidSizeR * 4;
                    if (ydelta * this.allRows.length > this.hSize)
                        ydelta = this.hSize / (this.allRows.length + 0.5);
                    var y = this.hSize / 2;
                    if (this.allRows.length % 2 == 0)
                        y += ydelta / 2;
                    for (var i = Math.floor(midN) - 1; i >= 0; i--) {
                        y -= ydelta;
                        this.svgCanvas.SetGroupPositionValue(this.allRows[i].groupTag, fLeft.width, y);
                    }
                    y = this.hSize / 2;
                    if (this.allRows.length % 2 == 0)
                        y -= ydelta / 2;
                    for (var i = (this.allRows.length % 2 > 0) ? Math.floor(midN) + 1 : midN; i < this.allRows.length; i++) {
                        y += ydelta;
                        this.svgCanvas.SetGroupPositionValue(this.allRows[i].groupTag, fLeft.width, y);
                    }
                }
            };
            NumberRack.prototype.OnClickAddRow = function () {
                if (this.allRows.length >= 10)
                    return;
                if (this.removedRows.length > 0) {
                    var row = this.removedRows.pop();
                    this.svgCanvas.AddGroupExists(row.groupTag, 0, 0);
                    this.allRows.push(row);
                }
                else {
                    var row = new math.RackRow(this.svgCanvas, this.wSize, 10);
                    this.allRows.push(row);
                }
                this.ArrangeRows();
            };
            NumberRack.prototype.OnClickRemoveRow = function () {
                if (this.allRows.length <= 1)
                    return;
                var deleted = this.allRows.splice(this.allRows.length - 1, 1)[0];
                this.svgCanvas.RemoveGroup(deleted.groupTag);
                this.ArrangeRows();
                this.removedRows.push(deleted);
            };
            NumberRack.prototype.CanAddRow = function () {
                return true;
            };
            NumberRack.prototype.CanRemoveRow = function () {
                return true;
            };
            NumberRack.prototype.DeleteChecker = function () {
                return false;
            };
            NumberRack.prototype.Init = function () {
                var _a, _b;
                (_a = upred.ui.uiModule) === null || _a === void 0 ? void 0 : _a.AddCustomModifier('removerow', {
                    img: './asset/icon_remove.png',
                    txt: '줄 삭제',
                    handler: this.OnClickRemoveRow.bind(this),
                    checkHandler: this.CanRemoveRow.bind(this),
                    alwaysOn: true
                });
                (_b = upred.ui.uiModule) === null || _b === void 0 ? void 0 : _b.AddCustomModifier('addrow', {
                    img: './asset/icon_add.png',
                    txt: '줄 추가',
                    handler: this.OnClickAddRow.bind(this),
                    checkHandler: this.CanAddRow.bind(this),
                    alwaysOn: true
                });
                ...
                var fLeft = document.getElementById('FRAME_LEFT').getBoundingClientRect();
                var fRight = document.getElementById('FRAME_RIGHT').getBoundingClientRect();
                this.wSize = RW - fLeft.width - fRight.width;
                this.hSize = RH;
                this.bidSizeR = 45;
                if (this.wSize / (this.bidSizeR * 2) < 21)
                    this.bidSizeR = Math.floor(this.wSize / 42);
                if (this.hSize / (this.bidSizeR * 2.2) < 10)
                    this.bidSizeR = Math.floor(this.hSize / 10 / 2.2);
                this.MakeBid('bidRed', this.bidSizeR, 'bidRedFill');
                this.MakeBid('bidWhite', this.bidSizeR, 'bidWhiteFill');
                window.addEventListener('pointermove', this.OnDragging.bind(this));
                window.addEventListener('pointerup', this.OnDragEnd.bind(this));
                this.InitBoard();
                upred.ui.uiModule.DisableDuplication();
                upred.ui.uiModule.DisableRotation();
                upred.ui.uiModule.uiButtons.select.tag.style.display = 'none';
                upred.ui.uiModule.uiButtons.delete.tag.style.display = 'none';
                this.OnClickAddRow();
                this.OnClickAddRow();
                window.setInterval(this.MoveContinue.bind(this), 40);
                upred.ui.GuideViewer.Show('./asset/guide.png');
            };
            return NumberRack;
        }());
        math.NumberRack = NumberRack;
    })(math = upred.math || (upred.math = {}));
})(upred || (upred = {}));
//# sourceMappingURL=numberrack.js.map